...
| T5 | **A cell's `color` is set by `.rux-table td` at specificity (0,1,1).** A severity or state class applied to a `<td>` is (0,1,0) and loses — and it loses *silently*, rendering default-coloured rather than erroring. Qualify it (`.your-block__table td.your-block__state--warn`). This shipped unnoticed in the Driver Roster and was caught by reading computed styles, not by looking at it. |
