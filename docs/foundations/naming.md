...
**Contract version: 1.17.0** · Stamped at the top so a downstream document can state the
version it conforms to. Authority without a version is only "whatever `main` says today,"
which is not control. See [`README.md`](README.md) §2.

**Status** · 15 steps: **14 done · 1 open**
...
| 15 | `.rux-menu` is a list surface with a menu's name | **[open]** | **Opened 2026-08-22 by `state.md` step 9, one turn after this document was declared complete** — which is what a finding does, and is recorded plainly rather than by quietly leaving the count alone. **Measured:** `.rux-menu` carries **four different roles** in `index.html` — `role="menu"` (`:341`, `:703`), `role="dialog"` (notifications, `:462`), `role="radiogroup"` (fleet filter, `:4924`) and a bare `<nav>` (documents, `:5385`) — and `menu.css` makes **no role assumption anywhere**. The block is a visual list surface; only `menu.js` is menu-specific, and it correctly scopes itself to `[role^="menuitem"]`. **So the name asserts a role its users do not share**, which is the same defect class as rule 2.4's one-name-two-concepts, one level up: a *block* name claiming semantics the block does not own. **This is a Class C rename and stops and proposes.** Candidate names want the §5.1 conformance program's eye rather than a guess here — Geist has no counterpart for a bare list surface (§7.3 table D lists `Menu`, and this is not only that). **Do not bundle it with the element rename** `../portability-audit.md` step 5 still owes. **Also worth deciding at the same time:** whether `.rux-notifications` and the documents nav should keep borrowing the surface at all, or whether the surface should be published under a neutral name and `.rux-menu` become a thin composition of it. That is the question §5.1's *retire, rename, migrate* order exists to answer, and this step belongs in that phase. |
