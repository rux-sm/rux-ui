...
*(R1. **Defined 2026-08-22, not yet enforced** — the definition is step 2, the six splits it
identifies are step 13, and the test is step 14. See §4 D1.)*
...
**R1 — the shape holds; the substance is now defined but still unchecked.**
...
2026-08-22 is that the sentence became checkable: rule 2.1 now states **ownership** and
**subject**, the two concepts its original wording lacked, and under that definition the
repository has **six** splits — `.rux-badge`, `.rux-button` and `.rux-panel` in the portable
layer, `.sched-scheduler`, `.sched-scope-right-panel` and `.sched-scope-trip` in the
application layer. **The test is deliberately not written yet**: shipping it now would mean
six allow-list entries, and `CLAUDE.md` is explicit that an exception list is not a passing
check. Step 13 removes the six; step 14 ships the test with an empty list.
...
| # | Defect | Status |
|---|---|---|
| D1 | ~~Rule 2.1's operative half has no *definition*.~~ **Defined, step 2 (2026-08-22)** — ownership plus subject. The remaining half of the defect is that it has no *test*, which is gated on removing the six splits the definition finds rather than on any further thinking. | **definition closed, step 2**; enforcement → steps 13, 14 |
...
| 13 | Consolidate the six R1 splits rule 2.1 found | **[open]** | Gated on nothing — step 2 defined the rule and named the six. Each is the job **step 6 already proved on `fleet-app`**: move a block's parts into one file of its owning layer, then verify by hashing every resolved selector's computed properties before and after and showing them identical to the digit. That step is also the argument that this is not tidying — consolidating `fleet-app` uncovered a `.fleet-app__order` rule that had been dead for months because a later file redeclared it at equal specificity. **Portable layer:** `.rux-badge` (`badges.css` + `notifications.css`), `.rux-panel` (`drawer.css` + `panel.css`), `.rux-button` (`controls.css` + `preferences.css` + `ui-header.css`). **Application layer:** `.sched-scope-trip` (2 files), `.sched-scope-right-panel` (2), `.sched-scheduler` (3). **Suggested order is smallest first** — `.sched-scope-trip`, then the two-file portable ones, with **`.rux-button` last**: three files and 334 occurrences make it the one most likely to turn up a real specificity dependency, and it should be attempted once the template has been re-proven on cheaper blocks. **Classification per block, not for the step**: a pure move is Class A, but any block whose resolved values shift is Class B and owes before/after in px. |
| 14 | Enforce rule 2.1 | **[open]** | **Blocked on step 13, and the block is the point.** The test is a short one — group each block's part-declarations by owning layer and assert no block appears in more than one file of its own — but shipping it before step 13 means six allow-list entries, and `CLAUDE.md` is explicit that a rule needing entries to pass is measuring the entries. **The acceptance criterion is an empty exception list**, not a green run. It SHOULD cite §2 rule 2.1 the way the other contract suites cite theirs, and it should be verified to fail when violated before being accepted — the discipline step 21 of `../portability-audit.md` recorded. **One residual risk to carry into it:** the ungrouped-selector clause means a genuine split hidden inside a grouped selector would not be caught. That is a deliberate trade — the alternative flags every shared rule in the layer — and it is recorded here so the test's blind spot is known rather than discovered. |
