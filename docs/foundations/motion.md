...
| | | | ~~`--rux-duration-productive`~~ | **retired** — ledger 25 |

**Nothing bridges them any more.** `--rux-duration-productive` resolved into the Carbon set,
had **zero** readers, and was retired by `../portability-audit.md` entry 25 for exactly that
reason: a dead alias between two scales is not neutral, it asserts they are interchangeable,
and Q1 established they are not. Every rung on the right is its own value: 80 is not 70, 140
is not 110, 220 is not 240. Reading this table as "two names for one scale" is the mistake it
exists to prevent.
...
**One token was dead and is gone.** `--rux-duration-productive` had **zero** `var()` consumers and
was retired (ledger 25). `--rux-motion-duration-moderate-02` also has none and **stays** — an
unused rung of a complete ladder is not a defect, per ledger entry 16.
