...
| 3 | **Consolidate** — move `../motion.md`'s Foundation Tokens table here; convert it to a pointer | **[open]** | The closing step, **unblocked by step 2**. Both scales survive, so the table moves here whole rather than being pruned to a winner — and it must gain the short set, which that document does not mention at all despite the layer using it 139 times. In scope: `../motion.md` § Foundation Tokens, and `../audit/design-system-audit.md` §5's rule 6, whose duration/easing half is one of the last two entries still stated there. **Not gated on step 4**: a table can name both scales correctly before either is renamed. |
