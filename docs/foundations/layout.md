...
**Q6 — Does a `cards.md` want to exist? — ANSWERED: it already does, at the component tier, and no family foundation is needed.** *Answered 2026-08-23 with step 26, which also corrected this question's number: step 17 appended it as a second "Q5" at the end of the file, outside §6, while §6 already held a Q5 — two questions, one number, found when `composition.md` Q4 cited "Q5" and a grep returned the wrong one. `docs/cards.md` (277 lines) predates the question and now carries the component-tier governance header from composition.md step 7; §9.1 keeps the card's *rules*, the tier doc keeps the *recipe* — the same split `foundations/motion.md` established for `../motion.md`. Original text follows.*

**Q6 (original, as "Q5") —** Step 17 states a card rule in §9.1 because that
