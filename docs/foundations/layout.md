...
**Contract version: 2.10.0** · Stamped at the top so a downstream document can state the
version it conforms to. Authority without a version is only "whatever `main` says today,"
which is not control. See [`README.md`](README.md) §2.

**Status** · 25 steps: **25 done**
...
| 18 | Recess the card footer — step 17's deferred half | **done · Class B** | **Executed 2026-08-23.** Step 17 took the rule and the banded background off card headers and recorded that Geist earns its *footer* separation with a border **and** a darker surface, which this system did not: `--rux-card-footer-bg` was `transparent`, so a footer was a rule and nothing else. **Before → after, resolved, both themes.** Dark: footer background `transparent` → `oklch(0 0 0)` against a body of `oklch(0.1457 0 0)`. Light: `transparent` → `oklch(0.9848 0 0)` against a body of `oklch(1 0 0)`. `border-top` is unchanged at 1px in both. **One token value moves, no name does:** `--rux-card-footer-bg` `transparent` → `var(--rux-surface-0)`, which is the page rung and therefore exactly the one-step-below-the-body relationship Geist's specimen shows. Step 17 is what freed that token: `--rux-surface-0` was the header's background until then, and using it for both would have made header and footer the same weight. **Blast radius is two instances today** — `index.html` and `js/panels/team-chat.js` are the only markup carrying `.rux-card__footer` — which is why this lands as a small change and not a sweeping one. **States needing an eyeball:** those two, plus the itinerary's dwell summary, which moved into a stop's footer in the same change and is the first real consumer of this treatment. Checked on screen in both themes. |
| 19 | A header shares the body's inline inset | **done · Class B** | **Executed 2026-08-23**, found by the owner looking at the result of steps 17 and 18 and saying the headers read as though they sat outside the card body. They did. `--rux-card-header-padding` was `0 0 0 16px` — **no inline inset on the right at all** — so a header's actions sat flush to the card edge while every body sat 16px in. That had been invisible for as long as the header carried its own background band, which drew the eye to the band rather than to the misalignment; step 17 removed the band and left the fault in plain view. **A change that looks correct can expose an older one, and this document should say so rather than let step 19 read as a regression from step 17.** **Before → after:** `0 0 0 var(--rux-space-4)` → `var(--rux-space-0) var(--rux-space-4)`, i.e. padding-right **0 → 16px**, every other edge unchanged. **The block inset stays 0 and that is deliberate**, not an oversight: `--rux-card-header-height` is a border-box *floor*, a header carrying a 32px icon button has only 7px of block room inside it, and `tests/ui-shell.test.mjs` already pins that reasoning — the subtitle case takes its block inset from `.rux-card__header:has(.rux-card__subtitle)` instead. That test pinned the old four-value form and was updated to assert the contract rather than the literal: block 0, inline `--rux-space-4` on both sides. **Verified in the running app:** header padding `0 16 0 16` against a body of `16 16 16 16`, insets matching, header height unchanged at 40px. |
| 20 | Claim the panel header band; correct its touch-block entry | **done · Class A + Class B** | **Executed 2026-08-23.** The owner raised the panel header from 40px to 64px, gave it a 20px inline inset and moved its title to heading-20 to match Geist's Modal, then asked for a review. Two of those needed no change and one was a defect. **Class A —** `--rux-panel-header-height` was never published in §10 even though its sibling `--rux-workspace-header-min-height` was, so a 24px change to a shared band had nothing recording it. §10 carries it now. **Class B —** §1.1's `max-width: 500px` block set it to **44px**, and that block exists to *grow* touch targets: with the floor at 64px it was shrinking the panel header by 20px on exactly the screens where it should never get smaller. Removed from the block; the floor already exceeds 44. **Before → after at ≤500px: 44px → 64px.** Every other token in that block still grows correctly — card header, UI header, tabs and workspace header all measured at 44px afterwards. **The title moved to heading-20**, which is applying rule 2.12 rather than amending the catalog: a panel header names a whole surface where a card header names a section inside one, and Geist sets its Modal title at `text-heading-20`. That required splitting `.rux-panel__title` out of the three-class recipe `typography.md` step 51 left in `card.css`, and **nine titles that were borrowing `.rux-card__title` inside a panel header were renamed to `.rux-panel__title`** so the role follows from the class rather than from position. Measured after: panel title 20/26/600 at −0.4px tracking, card titles unchanged at 16px. **Reviewed and left alone:** removing `.rux-card__title`'s standalone `padding-left` shifts no rendered text except where it should — the panel title nets +4px right against the header's new inset, and stop titles tighten 16px, closing a 24px gap after the pin to 8px. **Still open:** panel headers now inset 20px where card headers inset 16px. Geist uses 20 for its Fieldset, so the card may be the one that is wrong, but that is a separate step. |
| 21 | Attached panel headers align with their own bodies | **done · Class B** | **Executed 2026-08-23.** Step 20 left this open as "panel headers inset 20px where card headers inset 16px," and a sweep of all eleven panel headers found the real split was not panel-vs-card but **attached-vs-floating**. `.rux-panel--attached > .rux-panel__header` sets the `padding` **shorthand** from `--rux-panel-attached-header-padding`, which beats the base header's `padding-inline`, so step 19's inline inset reached floating panels only. Measured against each panel's own left edge with a `Range`, not the border box: the attached **title text sat at 16px while the cards in its body sat at 20px** — a header misaligned against the surface it names. Floating panels measured 21/21, aligned. **Before → after, all four attached headers: `8px 16px 8px 16px` → `8px 20px 8px 20px`.** Block inset unchanged at 8px; header height unchanged at 64px; title text and card edge now both 20px. **The fix is a reference, not a new literal:** the token's inline half now reads `var(--rux-panel-header-padding)`, so the inline inset has one home and cannot fork again. **Not a regression from step 20** — neither token was touched by it; step 20 fixed the floating half of a pre-existing split and this closes the other half. **States needing an eyeball:** Table Options rail (Drivers and Calendar), Documents list. Confirmed at 1280px; the token has a single definition and no theme override, so light and dark cannot differ. **Deliberately not done:** the flip-seven game panel's header grows to **80px at 375px** — its header carries a 176px cluster beside the title under `flex-wrap: nowrap`. That is a header-content problem, not an inset one, and it belongs to a module outside this system's surface. Card headers were also left at 16px: whether a card should follow Geist's 20px Fieldset inset is still the separate step 20 named. **Consumer exposure checked, because a Class B passes every name-based gate:** the `infor-ln-docs` portal vendors this token and still ships the 16px value, but it renders **zero** `.rux-panel__header` elements, so the change is inert there and stays inert after the next re-sync. Consumer gate clean against `portal/`. |
| 22 | The card's inline inset moves to 20px; §9.1 corrected | **done · Class B** | **Executed 2026-08-23**, closing the item steps 20 and 21 both left open. The owner asked for card headers at 20px to match the panel header and Geist's Fieldset. **Moving the header alone would have broken step 19** — a header shares the body's inline inset, and both measured 16px, so they were aligned before and would not have been after. The body's inline inset moves with it. **Before → after: `--rux-card-header-padding` `0 16px` → `0 20px`; `--rux-card-padding-inline` `16px` → `20px`; `--rux-card-padding-block` unchanged at `16px`.** Measured after in the trip editor at 1280px and 375px: card header `0px 20px`, body `16px 20px`, header title and first body label both at 21px from the card edge (20 + the 1px border), and in the itinerary the card title, stop marker, label, field and segmented control all resolve to the same 21px edge. **Inline 20 / block 16 is deliberate and is now stated in §9.1's lead rather than left to be read off the tokens:** the inline inset is the vertical edge the eye tracks down a column, which Geist sets at 20; the block rhythm is the gap between stacked things, where 16 is still the step. **Correction of the record:** §9.1's "Panel pane content inset | 16px" row was **never true**. `--rux-panel-padding` moved to 20px on 2026-08-16 in `33e2215`; §9.1 was written on 2026-08-22 and recorded 16px, so the document shipped stale for the row that would have revealed this whole split six days earlier. That row now reads 20px. **Blast radius:** 47 card headers and every card body, plus the four scheduler features that deliberately read `--rux-card-padding-inline` to stay in rhythm — itinerary, trip-manifest, trip-panel, flip-seven. Nothing clips at 375px; the card loses 8px of content width and the paired date fields still fit. **States needing an eyeball:** the itinerary tab and any dense two-column field row at 375px. Both tokens have a single `:root` definition with no theme or media override, so light and dark cannot differ and the value does not change with width. **Deliberately not done:** the block inset stays 16px — moving it would change vertical density on every card in the app, which is a different decision from the edge the owner asked about, and Geist's own Fieldset was not measured on that axis. Sibling-card gaps, stacked content rows and card-section regions stay at 16px for the same reason. **Consumer exposure checked**, since a Class B trips none of §4's name-based gates: the `infor-ln-docs` portal renders **zero** `.rux-card` elements, so this is inert there. It is also pinned far enough back that it still ships the pre-step-17 four-value `--rux-card-header-padding` (`0 0 0 16px`), which is a separate re-sync question and not one this step creates. Consumer gate clean. |
| 23 | The card's block inset follows to 20px | **done · Class B** | **Executed 2026-08-23**, immediately after step 22 and at the owner's direction. Step 22 held the block inset at 16px on the reasoning that it was a separate decision; asked directly, the answer was to move it. **Before → after: `--rux-card-padding-block` `16px` → `20px`.** A card body now resolves to a flat `20px` on all four edges, and `--rux-card-padding` collapses to a single value. Measured in the trip editor: body `16px 20px` → `20px`, **card height 362px → 370px** (+4 top, +4 bottom), header padding and the 40px header band both unchanged, since `--rux-card-header-padding`'s block half is 0 and the band is governed by `--rux-card-header-height`. **§9.1's seam warning re-checked, because this is exactly the combination it warns about:** header bottom padding 0 + body top padding 20 = **20px**, not 40 — the header carries no block padding to add. **`--rux-card-content-gap` deliberately stays at `--rux-space-4`.** That is the gap between things stacked inside the card, not the card's inset, and steps 22 and 23 together make that the operative distinction — which is why §9.1's lead was **rewritten rather than patched**. Step 22 had stated inline-20 / block-16 as a deliberate split along the horizontal/vertical axis; one step later that framing is wrong, and leaving it to be read alongside a table it no longer describes would be worse than replacing it. The rule is now inset (20) versus gap (16), and the two sibling-gap rows are labelled as gaps so the distinction survives the next reader. **Verified at 375px:** body `20px`, card 301px wide, paired date inputs 126px each, **zero children overflowing their card** and no horizontal document scroll. Both tokens remain single `:root` definitions with no theme or media override. **Deliberately not done:** panel panes were not touched — `--rux-panel-padding` is already `0 20px 20px 20px`, so panes and cards now agree at 20 without further change, and its `0` top is the attached-navigation seam §9.1's pane row already qualifies. Consumer renders zero `.rux-card`, so this is inert there as in step 22. |
...
**Rux insets card and panel content 20px — `--rux-space-5` — on every edge, and spaces
things stacked inside that inset 16px — `--rux-space-4`.** The distinction is between an
*inset* and a *gap*, not between horizontal and vertical: 20px is the margin between a
surface's edge and its content, matching Geist's Fieldset; 16px is the rhythm between
siblings within it. Steps 22 and 23 moved the two card axes to 20 in that order, and §9.1
briefly stated them as deliberately different — that reading is superseded. This describes
*relationships*, not unconditional padding on every edge, and it is not the only rhythm in
the app: §9.2 records the dense exception.

| Relationship | Contract |
|---|---|
| UI header to app shell | `0` |
| Panel to workspace | `0` |
| Panel pane content inset | `20px`, every edge — the seam clause died in step 25 |
| Sibling cards in a pane | `16px` — a gap, not an inset |
| Card header inset | `20px`, **both inline sides** — equal to the body's |
| Headered card body | `0 20px 20px` |
| Headerless card body | `20px` on all sides |
| Content rows inside a stacked card body | `16px` — a gap, not an inset |
| Card-section regions | same visual rhythm as cards |

**A header is not a separate surface; a footer is.** Header and body are one region: the
header carries no rule beneath it and no background of its own. The **footer** is the one
region a card separates, and it earns that with a top border *and* a background a step
below the body. A card that separates its header instead is spending contrast where Geist
spends none. Steps 17 and 18.
