...
**Contract version: 2.7.0** · Stamped at the top so a downstream document can state the
version it conforms to. Authority without a version is only "whatever `main` says today,"
which is not control. See [`README.md`](README.md) §2.

**Status** · 22 steps: **22 done**
...
**Rux insets card and panel content 20px inline — `--rux-space-5` — and spaces it 16px
block — `--rux-space-4`.** The two axes are deliberately different, and step 22 records why:
the inline inset is the vertical edge a reader's eye tracks down a column of headers, labels
and fields, and Geist sets that edge at 20 on its Fieldset; the block rhythm is the gap
between stacked things, where 16 remains the step. This describes *relationships*, not
unconditional padding on every edge, and it is not the only rhythm in the app: §9.2 records
the dense exception.
...
| Headered card body | `0 20px 16px` |
| Headerless card body | `16px` block, `20px` inline |
