...
**Contract version: 6.1.0** · Stamped at the top so a downstream document can state the
version it conforms to. Authority without a version is only "whatever `main` says today,"
which is not control. See [`README.md`](README.md) §2.

**Status** · 71 steps: **67 done · 1 deferred · 3 withdrawn**
...
specimens on 2026-08-21 (49). **What is left is not conformance:** the app-tier defects are closed — D19 by
`../trip-bar.md` step 10 (its orphaned rung retired by step 68), D22 by step 69, D17 by
step 71, and D10 by steps 50–51 before the counter could see it. **D8 alone stays open**,
deferred on step 4's reasoning; steps 4, 23, 32, 34 and 46 wait on a consumer.
...
size to pair with and one reader, the trip-bar pill's default tier (D19) — until
`../trip-bar.md` step 10 deleted the pill and step 68 retired the rung. `wide`
...
Every rung is a catalog value, named by its number per Q8 — including the leading ladder,
since step 68 retired `12`, the one rung the catalog did not have.
...
### 3.5 The container ladder

§3.2 says what each role *is*. This says which role a **container's own title** takes. The
mapping was unpublished until step 64, and the cost of that is measurable: a census of the
reference application found **92 of 121 framed boxes hand-rolled rather than `.rux-card`**,
and three separate hand-built versions of "a titled group" — `settings-app__panel` (×8),
`sched-tasks__day-title` (×1), and `.rux-section` itself, which shipped with **zero callers**
until 2026-08-23.

| Container | Titles | Role | px |
|---|---|---|---|
| UI header | **the module** — persistent chrome, outside the ladder | `text-heading-16` | 16 |
| Workspace | **what is being viewed** — *no current wearer; the class was removed, step 66* | `text-heading-24` | 24 |
...
heading sits **outside** the frame it introduces; a card title sits **inside** one. That
difference is already unambiguous, and spending a size step on it would either push the
section above the panel that contains it or collapse the panel and section together.

Two independent authors reached this conclusion before it was written down — `tasks-panel.js`
and `settings-app` both chose 16 for a heading over a run, neither having a rule to follow.
...
| **D10** | **Fixed by steps 50–51, struck by step 70 — the row simply outlived its repair.** ~~The nine Tier 2 utilities are defined beside whichever component first needed each one. There is no single published index, so a call site cannot discover them.~~ The 29 role utilities live in `base/text.css`, the object-named ones in `base/utils.css`, and §3.3's table documents the one exception (`.rux-u-subtitle`, `card.css`) with its reason. | §3.3 |
...
| **D22** | **Fixed (step 69), plus a twin found beside it.** ~~`scheduler/css/features/maintenance-share.css:29` sets `font-size: 12px` as a literal~~ on `.maintenance-trip :is(strong, span, small)` — a rule 2.1 defect in the app tier, found by step 52's grep for rules naming `strong`. One token swap (`--rux-text-label-12-size`) with no render change; recorded rather than swept because it is outside the step that found it. | `maintenance-share.css:29` |
...
| 66 | Remove `.rux-workspace__title` and `.rux-workspace__subtitle` | **done · Class C** | **Executed 2026-08-23 with the owner's explicit authorization**, after the proposal the rename protocol requires. The title's last wearer retired with the Documents records table (`composition.md` step 6); the subtitle's went with Settings' orphan heading. **Grep before (occurrences per location):** index.html 0 · js 0 · scheduler/css 0 · rux-ui/css 3 (one rule + two comments) · tests 6 · docs 12 (log history, which stays). **Removed:** the `workspace.css` rule pair, `.rux-workspace__subtitle` from `card.css`'s shared subtitle group, and the two stale comments — `text.css`'s step-51 note also stopped claiming `.rux-panel__title` reads heading-16 there, which step 20 had already made false. **§3.5's workspace row is kept, marked *no current wearer***: the heading-24 rung is knowledge — what a workspace title costs if one returns — while the class was dead weight; the ladder enforcement drops the rung with a comment saying why, and a retirement test asserts the class defines nothing and appears nowhere. **Grep after:** rules 0 everywhere; remaining hits are tombstone comments and this log — **after a correction**: the first re-grep missed `examples/app-layout.html`, which still wore the title class, because the rename protocol's location list (index.html, js, tests, docs, CSS) does not name `examples/`. `tests/class-resolution.test.mjs` caught it — R5 doing exactly its job — and the example was cleaned in the same step. The protocol gap is the finding: a sweep that follows the list verbatim misses example markup. **Consumer:** ships the old rules until re-sync and renders zero of either class — the removal reaches it as a no-op at the next vendor sync, recorded there when it happens. **Major bump** (4.13.0 → 5.0.0): a public class name was removed. |

| 67 | Defect bookkeeping repair — §4 was invisible to the rollup counter, and it was hiding five live defects | **done · patch** | *(Recorded as step 63 until 2026-08-24 evening — a duplicate number; 63 was already the Q2 correction. Renumbered by step 68, which found the collision; the commit message that says "step 63" is immutable and means this row.)* **Executed 2026-08-24.** §4 marked its numbers `**D1**` where `tests/foundations-contract.test.mjs` matched plain `| D1 |`, so **all 22 rows were skipped entirely** — not counted open, accepted or resolved. This document published **0 open defects** while carrying **five**: **D8** (no measure token), **D10** (nine utilities with no home), **D17** (`.sched-scheduler__time-ticks` duplicating `display: none`), **D19** (the trip-bar pill at 12/12), **D22** (a `12px` literal in `maintenance-share.css`). Struck the seventeen the log already records as fixed, so the counter reads resolved from `~~` rather than calling every *"Fixed (step N)"* row open. **The counter was fixed in the same change** to accept both spellings — this is the third document to hit the same bug after `forms.md` step 4 and `color.md` step 21, and a checker that silently ignores what it cannot parse is worse than no checker, because the number it prints looks the same either way. **Deliberately did not** act on any of the five: each is a real defect with its own cost, and surfacing them is not the same as choosing to fix them. **Deliberately did not** unbold the ids — with the counter accepting both, the emphasis is now a style choice rather than a trap, and rewriting 22 rows to satisfy a regex was the failure mode, not the fix. |
| 68 | Retire `--rux-line-height-12` (D19's orphan) | **done · Class C** | **Executed 2026-08-24 under the owner's explicit order, after the reader died.** The rung was minted for `size-11` (retired, step 45), then survived on one reader — the trip-bar pill's hand-set 12/12 leading (D19) — and `../trip-bar.md` step 10 deleted that pill entirely, leaving the rung reader-less. **Grep protocol, before:** 1 declaration (`rux-ui/css/tokens.css:251`), **0 reads** anywhere — `index.html`, `js/`, `tests/`, all CSS, all consumer-facing pages; the only other mentions are step-history prose in three documents, which stays as written. **After:** 0 occurrences outside history. **Removed outright, no alias window, deliberately:** step 31's aliases forwarded a renamed rung to the *same* value; this rung has no successor — 12px text pairs with `line-height-16` per the Label catalog, and an alias pointing 12→16 would silently grow any consumer's leading by a third, which is worse than the loud break the distribution gates exist to produce. A vendored consumer that reads the name will trip `design-system-distribution.md` §4's name gate on its next sync, which is the gate doing its job. **The leading ladder now matches the measured catalog exactly** — this was its last off-catalog rung (§3.1 updated). **Found and fixed in passing:** the log carried **two rows numbered 63** — the bookkeeping repair reused the Q2 correction's number; renumbered to 67 with a note, since a log whose numbers collide cannot be cited. Contract 5.0.0 → **6.0.0** (Class C takes a major). |
| 69 | Replace the `maintenance-share.css` literals (D22) | **done · Class A** | **Executed 2026-08-24 under the owner's order.** `font-size: 12px` → `var(--rux-text-label-12-size)` at the recorded site (`:29`, the `strong/span/small` rule) **and at its twin four lines up** — the `.maintenance-trip` base rule carried the same literal, the same rule-2.1 shape, unrecorded; fixing the named line and leaving its neighbour would have been the letter of the defect over its point. No render change: the token resolves to the same 12px. |
| 70 | Strike D10, which steps 50–51 had already fixed (bookkeeping) | **done · patch** | **Executed 2026-08-24.** D10 described the pre-step-50 world — role utilities scattered beside whichever component first needed them — and steps 50–51 ended it: 29 role utilities in one file imported last, object utilities in `base/utils.css`, one documented exception. Verified by grep, not memory: 29 `.rux-text-*` rules in `text.css`, 8 `.rux-u-*` in `utils.css`, 1 in `card.css`, exactly as §3.3's table says. The row survived because §4 was invisible to the rollup counter until step 67 — this is the strike that repair deliberately left to a decision. **Of the five defects step 67 surfaced, the ledger now reads:** D19 fixed by deletion (`../trip-bar.md` step 10, struck at 68), D22 fixed (69), D10 was already fixed (this step), **D8 stays open-and-deferred on step 4's recorded reasoning** — a measure token with zero readers is what §7.3 forbids, and that reasoning was re-read and still holds — and **D17 awaits the owner's reveal-or-delete call**, now with rendered evidence (the ticks were flipped visible in a live browser: 20 of 21 render as faint hour marks — 12/6/6 — on the day-head baseline in time-aligned mode; the 21st is hidden by design). Contract 6.0.0 → **6.0.1**, shared with step 69. |
| 71 | Reveal the time-aligned hour ticks (D17) | **done · Class B, app tier** | **Executed 2026-08-24 on the owner's call — the last of the five defects step 67 surfaced.** `.sched-scheduler--time-aligned .sched-scheduler__time-ticks` read `display: none`, the same value as the base rule it exists to override, so **42 DOM nodes were unreachable in every state**; it now reads `display: block`. **Before → after, measured through the real control** (the Time-aligned switch in Calendar Options, not a forced class or an injected stylesheet): 0 of 21 ticks rendered → **20 of 21**, each a 20×16px box of `label-12` in `oklch(1 0 0 / 0.28)` — the disabled tier — reading **12 / 6 / 6** along the day-head baseline. The 21st stays hidden by the `nth-child(1)` rule beside the bus rail, which is deliberate and predates this. **This is a behaviour change and says so:** anyone using time-aligned mode now sees hour markers that have never rendered, which is what the mode was built for. **Deliberately did not** re-tune their type, colour or offsets: step 42 already put them on `label-12`, and tuning a thing nobody has seen is guesswork — the eyeball comes first. **Not screenshotted:** the app's profile gate was up and dismissing it is the owner's state to set, so the evidence is computed styles through the real toggle rather than an image. The toggle was returned to the off state it was found in. Contract 6.0.1 → **6.1.0** — a minor, because an app-tier rendering changed while no published name or value moved. |
---
