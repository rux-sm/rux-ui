...
**Contract version: 1.4.2** · Stamped at the top so a downstream document can state the
version it conforms to. Authority without a version is only "whatever `main` says today,"
which is not control. See [`README.md`](README.md) §2.

**Status** · 8 steps: **8 done**
...
**A theme control is an option set, not a boolean.** *[Geist]* Geist publishes a Theme
Switcher whose canonical form is three options — Light, System, Dark — and states plainly:
*"don't rebuild a theme picker with Switch or three icon buttons."* Resolved into this
system's own vocabulary rather than by importing Geist's name: [`naming.md`](naming.md)
§7.3 already records that Geist's *Switch* is the option set (`.rux-segmented-track` +
`.rux-button--segment`) and its *Toggle* is the boolean (`.rux-switch`). So a theme control
built as a switch is the wrong control twice — wrong arity, and the one the source names as
the mistake. **Where it is placed is [`shell.md`](shell.md) § Action Placement's**, which
answers *where* as this section answers *what*. **D6** records what ships.
...
**Corrected 2026-08-24 by step 8, which read the 16 rather than counting them.**
A zero in that column is not 16 violations: **nine of the sixteen are a control inside
its own `<label>`** — `.rux-checkbox` rows in `tasks-panel`, a `.rux-switch` in the other
two — which is a labelled control, just not a field. **One was a genuine field** and is one
now (the post-trip note). **Six were unnamed or misnamed** and step 8 fixed them. The
column measures the wrong thing, and the number that matters is *controls with no
accessible name*, which is now zero across all three. Read this table with Q4 beside it.
...
| | |
|---|---|
| D1 | **Fixed (step 8), and its premise was wrong** — nine of the sixteen are a control inside its own `<label>`, which §2.1 does not reach; one wanted a field and has one; six were unnamed or misnamed and are fixed. ~~Three JS-rendered panels compose no fields at all — `tasks-panel`, `settings-panel`, `requirements-panel` render 16 controls between them and zero `.rux-field` wrappers, so §2.1 is violated wholesale rather than at the margin. Not yet a numbered step; the census above is the scope.~~ The zero was real and meant less than it read; §3 carries the correction. |
...
| D4 | **Fixed (step 7).** `tests/forms-contract.test.mjs` enforces §2.3, §2.4, §2.6, the block-class half of §1/§2.8, and content.md rule 2.3. ~~No test enforces any rule here. Every other foundation document has at least one contract suite; this one has none, and §2.1 and §2.2 are both mechanically checkable.~~ **The scoping in the original text was wrong and step 7 records why:** §2.1 is *not* mechanically checkable — it publishes no exemption vocabulary, which is now Q4 — and §2.2 is checkable only in its casing half, which is content.md rule 2.3's rather than this document's. |
| D6 | **The theme control is a boolean where the source publishes a ternary.** `.rux-switch--theme` (`index.html`, Preferences → Appearance) offers dark/light; Geist's Theme Switcher offers Light / System / Dark. **The gap is functional, not cosmetic**: `rux-ui/js/theme.js` follows the OS *only while no manual choice is stored*, and the first toggle writes `localStorage` — so **system-follow becomes unreachable the moment the control is used**, and nothing in the UI can restore it. `naming.md` §7.3 maps Geist's Theme Switcher to `.rux-switch--theme`; that answer is correct **as naming** and silent on arity, which is how a missing third state stayed invisible to a census that counted blocks. Fixing it is **Class B twice over** — the rendered control gains a state, and `theme.js`'s storage contract needs a `system` value meaning *stop reading the stored choice* — so it stops for a proposal. Not yet a numbered step. |
...
| # | Step | Status | Notes |
|---|---|---|---|
| 8 | D1 resolved — the three panels are named, and the suite learns to read `js/` | **done · Class A** | **Executed 2026-08-24.** D1 said three JS panels compose no fields and called §2.1 violated wholesale. **Reading the sixteen controls rather than counting them found a different defect**, and the smaller one. **Nine of the sixteen are a control inside its own `<label>`** — `.rux-checkbox` task rows in `tasks-panel`, a `.rux-switch` in `settings-panel` and `requirements-panel`, each wrapping its input beside the text that names it. That is a labelled control; it is not a field, and §2.1 does not reach it. Counting it as a violation is the same mistake as the 47 in Q4, one tier down. **§3's gap table is corrected in place rather than deleted** — the zero was true, and what it measured was not what it read as. **One control genuinely wanted a field and now has one:** the post-trip note. It was a bare `<textarea>` named by `aria-label` with an instruction placeholder — *Add a note…*, Geist's named anti-pattern — and is now `.rux-field` + a visible *Post-trip note* label, no placeholder, no `aria-label` (which would be §2.6's double). **`.sched-tasks__note-input` moved from the textarea to the field wrapper**, the same move step 7 made on `.rux-profile-picker__add-name` and for the same reason: the class is `width: 100%` + `margin-top` + `resize`, and the last two belong to the block while the first two are what `.rux-textarea` already gives. No CSS changed. **Six controls were unnamed or misnamed, and are fixed:** `settings-panel`'s billing label input sat beside a `<span class="settings-billing-row__label">` that named nothing — the span is a `<label for>` now, and clicking *Contract* focuses its input, verified live; its visibility switch and `requirements-panel`'s active switch were named only by `title`, which is a fallback, not a name; `requirements-panel`'s row input had no name at all; and `tasks-panel`'s requirement detail input carried `aria-label="${row.placeholder}"` — **the placeholder laundered into ARIA**, which is §2.3 with an extra step, now `${row.label} detail` on the panel's own convention. **The suite grew a second surface, which is what makes this stick.** Step 7 scanned HTML only and said so; D1 lives entirely in template literals, so the walk now runs over `js/` too. **A JS template resolves its own label pairs without evaluating anything**: the label's `for` and the control's `id` are the same unevaluated string, so matching them as text is exact. Two scanner defects were found by their false positives and fixed — a `//` comment reading as markup (`doc-viewer.js` and `trip-envelope.js` each explain that "a `<select>`" needs `boot()`), and a `for`/`id` of `tp-ticket-label-${index + 1}`, whose spaces sit inside the expression, being split into three targets and matching none. **§2.3 gained an assertion, not a rule:** an `aria-label` identical to the `placeholder` is the placeholder acting as the label. That is what §2.3 already says; the separate test exists so the failure message can name the shape. **Worth a sentence in §2.3 eventually** — recorded here rather than added, because rule text is not this step's to write. **Three more defects outside D1's three panels, found by the same widening and fixed:** `trip-db.js`, `trip-manifest.js` and `trip-panel.js` each render a payment-date input carrying `aria-label="Payment date"` inside a `<label class="rux-field">` that already says *Date*. Two names, disagreeing, adding nothing — the label wins and the `aria-label` is gone. **Three that are NOT fixed, and are Q5.** `trip-panel.js`'s two driver-row fields and `itinerary.js`'s resting address put a second name on a control deliberately — to tell three identical rows apart, and to carry state. §2.6 forbids both and publishes no alternative. They are recorded in `tests/forms-contract.test.mjs` as `KNOWN_DOUBLE_NAMED` on the `gallery-coverage` ratchet idiom, **with a second test that fails if a listed line stops being a double-name without coming off the list** — so the list cannot silently pin the wrong line. **Deliberately not done.** No `.rux-field` was added to the nine self-labelling controls; that would be the §2.1 reading Q4 exists to settle, and doing it here would answer Q4 by fait accompli. `aria-labelledby` composition for Q5 was not attempted — it needs ids on markup that has none. And the casing test stays HTML-only: a JS label is usually an interpolation, and its text is data. **Verified.** 422/422 green. `node --check` on all six edited modules. Live DOM at 8642: every billing row's label resolves to its input (`label.control === input`), the three switches announce *Show Contract in the trip editor* / *Sleeper active* / *56 Pax active*, and the requirement inputs announce *Requirement label*. **The post-trip note was measured, not seen in place** — it renders only for a trip in post-trip state, so its markup was mounted in its real ancestry and measured there (field margin-top 8px, gap 8px, 396px wide, textarea padding 8px, resize vertical) and removed; the block itself is unphotographed and that is the residual gap. Billing rows unchanged in height and layout after `<span>` → `<label>`, which grid blockifies either way. Cache-busters bumped by the tool: `trip-db` v43→44, `settings-panel` v11→12, `tasks-panel` v71→72, `trip-manifest` v12→13, `trip-panel` v46→47. **T1's split versions were not touched and no new split was made** — `trip-db.js` was already page-43 against script-24 and script-12, so `--fix` had no unified pair to break; `requirements-panel.js` loads with no `?v=` at all, which is why the audit never named it. No rule text, token, or name moves. Contract 1.4.1 → **1.4.2**. |
| 7 | D4 resolved — the rules gain a contract suite, and the app moves to meet it | **done · Class A** | **Executed 2026-08-24.** D4 said §2.1 and §2.2 were both mechanically checkable. **Neither is, as it scoped them**, and finding that out is most of what this step produced. **§2.1 cannot be tested until it says what it exempts** — 47 of `index.html`’s 135 controls have no `.rux-field` ancestor and roughly 37 of those are correct (a `.rux-switch` in its own `<label>`, a colour-swatch radio, a hidden file input, a stepper, a slider, a toolbar search). That is now **Q4**, and the suite deliberately asserts nothing about §2.1 rather than encoding a guess as a rule. **§2.2 is checkable only in its casing half, and that half is content.md rule 2.3’s** — step 5 moved the casing there precisely so it would be stated once, so the test cites content.md and not §2.2. The `noun` half needs part-of-speech knowledge; D3 stays open. **What landed:** `tests/forms-contract.test.mjs`, six assertions across all nine pages — §2.3 (a placeholder is never the only name), §2.6 (no second accessible name beside a real label; every `<label for>` resolves), §2.4 (a control’s `aria-describedby` resolves), §1/§2.8 (a `<select>` wears `.rux-select`, a `<textarea>` wears `.rux-textarea`), and content.md 2.3 (field labels are sentence case). **Two of the six were already green when written and are guards, not fixes** — `<label for>` resolution and `aria-describedby` resolution both measured zero. Said plainly rather than counted as work. **Nine live defects the other four found, all fixed in `index.html` so the suite lands green:** three controls named only by a placeholder (`profile-picker-add-name` “Display name”, `settings-req-new-label` “Label”, `tp-import-textarea`’s paste instruction — Geist’s named anti-pattern verbatim); four controls carrying `aria-label` beside a real label (`tp-price`, `tp-est-mi`, `tp-act-mi`, `tp-po-amount`), of which **`tp-est-mi` had two names that already disagreed** — visible “Est. Miles” against `aria-label="Estimated miles"`, which is the exact failure §2.6 predicts; one `<textarea>` wearing `.rux-input`; and two Title Case field labels, **“Est. Miles” and “Pick-up Location”, that step 5’s live-DOM sweep missed**. That miss is the argument for the test: step 5 checked 142 labels by hand and reported zero remaining, and a period and a hyphen defeated its heuristic. A census is a measurement; only a test is a guarantee. **One exception is recorded in the suite, not worked around:** `Google Messages URL`, step 5’s deliberate leave (proper noun + acronym). An all-caps word passes the rule by construction, so `Trip JSON` needs no exception. **This changes what renders, in three places, and each was eyeballed** on `tools/serve.mjs` at 8642 in both themes: the profile-picker add form gains a *Display name* label above the input (the field wrapper takes `.rux-profile-picker__add-name`’s `width: 100%`, since that rule is the whole class — measured after: form 338px, field 338px, input 338×36px, so nothing moved but the label’s arrival); the import modal gains a *Trip JSON* label and a `.rux-field__help` line wired through `aria-describedby`, replacing the instruction placeholder, and its textarea picks up the textarea padding and 20px leading it should always have had; the requirements add-row gains `aria-label="Requirement label"` and an example placeholder. **The placeholder was chosen twice** — `ADA lift` first, then `Restroom`, because the live list already contains an *ADA Lift* and an example that names an existing row reads as a value rather than a hint. **Deliberately not done.** No `.rux-field` was added to the three panels D1 names — that is D1’s own step and it is JS, not markup. D2’s 74 placeholders are still unsplit; this step fixed the three the *test* can identify (no other name), which is a different and smaller claim than reading all 74. The requirements row rendered by `requirements-panel.js` carries no accessible name either, and is left for D1’s step rather than fixed here under a cache-buster bump. And **no test asserts §2.1**, per Q4. **Verified:** 420/420 green; the four substantive assertions were re-run against the pre-fix `index.html` and each failed naming exactly the defects above, so none of them is vacuous. Census re-measured across `index.html` and `js/`: controls 208 (unchanged), `.rux-field` wrappers 151 → 153, placeholders 73 → 71, controls carrying `aria-label` 60 → 57. No rule text, token, or name moves, so the minor stays put. Contract 1.4.0 → **1.4.1**. |
| 6 | §2.8 gains the theme control; D6 records what ships | **done · Class A** | **Executed 2026-08-24**, found while answering a `shell.md` question — *where does the theme switcher belong* — that turned out to have a control half this document owns. **Geist publishes the answer and it was read off the rendered page, not recalled**, per this set's standing method: `vercel.com/geist/theme-switcher` renders a **three-option** specimen (system / light / dark) under the label *"Select a display theme:"*, and its best-practice list states *"don't rebuild a theme picker with Switch or three icon buttons."* **The rule resolves through this system's own vocabulary rather than importing Geist's name**: `naming.md` §7.3 already records the `switch`/`toggle` crossing — Geist's *Switch* is an option set, its *Toggle* is the boolean — so "a theme control is an option set" lands as `.rux-segmented-track` without adopting a foreign name, and §7.3's warning that the two names move together or not at all is untouched. **A prior answer is narrowed, not overturned:** §7.3 records Geist's Theme Switcher as *answered by* `.rux-switch--theme`. That is right as naming and says nothing about arity — the census enumerated blocks, so a two-state control could satisfy a three-state source without anything noticing. D6 carries the arity gap; the name mapping stands. **Deliberately not done: the app was not changed.** The rendered control gains a state and `theme.js`'s storage contract has to grow a `system` value that means *stop reading the stored choice* — Class B on both halves, which stops and proposes rather than riding along on a rule addition. **Deliberately not done:** no move of the control, and no new `--rux-*` token; where it sits is `shell.md`'s and it is already in a sanctioned home. D6 carries the scope. Contract 1.3.0 → 1.4.0. |
...
**Q5 — How is a repeated field named?** Asked 2026-08-24 by step 8, which found three
controls carrying `aria-label` beside a real label and could not call any of them a
mistake. Two are the driver-row fields in `trip-panel.js` — *Meet / swap time* and *Driver
note* render once per role, so the visible label is correct and identical three times over,
and the `aria-label` adds the role title to tell them apart. One is the itinerary's resting
address, where the accessible name carries **state** rather than identity: it announces
either the address or that the stop inherits the previous one.

**§2.6 forbids both and offers nothing in their place.** Its rationale is that two names
can disagree — but here the second name is deliberately more specific, and deleting it
makes a screen reader announce three identical fields. **The composed answer is
`aria-labelledby` naming the row title and the field label together**, which keeps one
source of truth and satisfies the rationale; it needs ids on markup that has none, which
is why this is a question and not a step. The state case is different again and may not be
a naming problem at all — `aria-describedby` onto help text is the usual answer, and §2.4
already publishes that mechanism.

**They are recorded, not excused:** `tests/forms-contract.test.mjs` carries the three in
`KNOWN_DOUBLE_NAMED`, and a second test fails if one stops being a double-name without
coming off the list — so the ratchet cannot rot in either direction.

**Q4 — Which controls does §2.1 exempt?** Asked 2026-08-24 by trying to enforce the
rule and finding it unenforceable as written. **47 of `index.html`'s 135 controls have no
`.rux-field` ancestor, and most of them are right**: a `.rux-switch` inside its own
`<label>` (14), a `.rux-color-swatch` radio (12), a hidden file input behind a button (4),
an `input[hidden]` (3), a `.rux-number-stepper` (2), a `.rux-slider`, and a toolbar search.
Roughly ten are genuine gaps. §2.1 names none of this, so a test cannot separate the two
and `tests/forms-contract.test.mjs` does not try — the rule is the blocker, not the test.
**What the answer has to produce is a predicate**, not a prose exemption: something a test
can evaluate. The candidates are *the control is its own labelled block* (`.rux-switch`,
`.rux-choicebox`, `.rux-color-swatch` — each already wraps itself in a `<label>`), *the
control is not user-visible* (`[hidden]`, the file input behind a button), and *the control
is a toolbar affordance rather than a form field* — which is the loosest of the three and
the one most likely to become a loophole. **§3's gap table reads narrower than the
truth and is worth re-reading with this question**: it lists three JS panels at 16 controls,
because it counts modules with *zero* wrappers. The 47 is the same defect measured
per-control, and it is where the answer will be tested.
