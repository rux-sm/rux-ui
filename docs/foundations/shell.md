...
- A workspace toolbar separates its **groups** with `--rux-button-group-gap`;
  controls *within* one group sit flush, nested in a single child. The default
  was flush until step 6, and it was backwards: three of the four multi-group
  toolbars in the application had already overridden it to the same value under
  two different token names. A default most consumers override is not a
  default. *(Step 6.)*
...
| D2 | **The attachment rule and the shared drawer mechanism contradict each other.** § Application Anatomy: *"Panels and the workspace MUST be attached with no decorative gutter between them. A separator or resize hit target MAY occupy their shared boundary **without creating a visible gap**."* § Right Panel repeats it: spacing goes *"around the combined assembly—not between the panel and workspace."* But `.rux-drawer-gutter` (`rux-ui/css/base/drawer.css`, promoted to Tier 1 by `../portability-audit.md` entry 11) exists to open exactly that gap, and `--rux-drawer-gap` defaults to `var(--rux-space-5)` = **20px** in `rux-ui/css/tokens.css`. **Measured live 2026-08-26**: in `index.html` the workspace's right edge sits at 1420px and the tools panel's left edge at 1440px — a **20px visible gap**, from the element the rule names as permitted. Found by rebuilding `../../examples/app-layout.html` against this document (step 7); the example renders the same 20px and says so in place, rather than choosing a local value that would hide it. **Not resolved here, and the two paths are not equal.** (a) *The rule is right and the mechanism is wrong*: `--rux-drawer-gap` drops to 0, which is Class B and moves every drawer boundary in the application and in the vendored consumers — no name changes, so none of `../design-system-distribution.md` §4's three gates would see it. (b) *The mechanism is right and the rule is under-stated*: a **disclosure** surface is not a **persistent attached** panel, and the rule was written for the second — in which case § Application Anatomy gains the distinction it is currently missing, which is Class A and moves nothing. (b) is the likelier reading, since every rendered instance has voted for it for months; it is still the owner's call, because writing it down legitimises 20px of gap the document currently calls invalid. |
...
| 6 | The toolbar gap default is inverted: groups separate, clusters nest | **done · Class B** | **Executed 2026-08-26**, found by the owner looking at the records example and asking whether the zero gap between the search field and the segmented track was intentional. It was published — `.rux-workspace__toolbar` carried `gap: 0` — and it was not defended anywhere. **The evidence is that the application had already voted.** Measured across every rendered toolbar: Calendar 8px, Requests 8px, Drivers 0px, Driver Roster 0px, Fleet single-child. The two at 8px got there by overriding the shared default in their own feature CSS — `scheduler-app.css` via `--rux-space-2` with the comment *"Flush cluster, but not flush against the panel toggle beside it"*, and `request-inbox.css` via `--rux-button-group-gap` with the comment *"this toolbar holds two distinct groups … so they take the standard button-group gap between them"*. Two consumers, the same reasoning, the same resolved 8px, two different token names. A third override had just been added in `driver-roster.css`'s narrow block. **Before/after resolved values**: `.rux-workspace__toolbar` gap `0` → `var(--rux-button-group-gap)` = **8px**. Calendar and Requests are unchanged in render — they already resolved to 8px and merely stop paying for it locally. **Drivers and Driver Roster change**: their controls stop touching. Fleet's toolbar has one child, so gap does not apply. **All three overrides are deleted**, which is the actual value of the step — the rule now has one home instead of three, and the semantic token `--rux-button-group-gap` wins over the two call sites that reached past it to the `--rux-space-2` primitive. **States still needing an eyeball**: the four toolbars at narrow widths, where the band wraps. **Deliberately not done**: `.rux-workspace__heading` keeps its own `gap: var(--rux-space-2)` rather than being unified — it stacks a heading and a subtitle vertically, which is a different relationship from separating control groups horizontally, and collapsing them would be a coincidence of value dressed as a rule. Contract 1.3.0 → **1.4.0**. |
...
| 7 | Rebuild the reference composition against this document; enforce it at composition level | **done · Class A** | **Executed 2026-08-26**, after the owner read `../../examples/app-layout.html` and said it *"doesn't follow or provide any useful info."* Measured before touching it, that was right on five counts, and **all five passed a green suite** — which is the finding, not the file. `tests/layout-contract.test.mjs` asserted the example by **name presence** (`assert.match(exampleHtml, /class="rux-app"/)` and eleven more of that shape), so it proved strings were in the file and nothing about what they did. **What had drifted out:** (1) `.rux-app-view` was absent entirely — 0 occurrences against 12 in `index.html` — so the shared inset frame § Right Panel makes a MUST had no instance in the page that exists to demonstrate it, and the workspace and panel ran flush to the window edge; (2) the panel was labelled `aria-label="Calendar Tools"` — this document's own named example — with **no trigger at all**, against § Right Panel's *"The trigger MUST expose aria-controls and aria-expanded"*; (3) the toolbar held two loose buttons and zero `role="group"`, so step 6's groups-separate/clusters-nest rule had no instance, in the file that should be its clearest one; (4) no `.rux-drawer` / `.rux-resize-gutter`, Tier 1 since `../portability-audit.md` entry 11; (5) the tab strip was **dead markup** — the page loaded only `ui-shell.js`, so clicking a tab moved no `aria-selected` and hid no pane, verified in the browser before the rebuild. Tabs also carried `aria-controls` with no tab `id` and no `aria-labelledby` back-pointer. **The rebuild composes the real mechanisms rather than restating them**: one `.rux-app-view` carrying the shared frame configured on `.rux-app`; the drawer-gutter/resize-gutter pair at the boundary; a workspace-header disclosure wired through `RuxDrawer.create()` **with no `configure()` call**, because the module's portable defaults already name this composition; and a nested flush cluster beside the toggle so the toolbar shows both halves of step 6. The old page's `@media (max-width: 760px) { display: none }` is deleted — `base/drawer.css` already owns the ≤500px full-viewport overlay, verified at 420px (drawer 420px, `position: fixed`, scrim visible). **Enforcement replaced, not extended.** The name-presence block became five composition tests that assert implications: a panel implies a trigger that names it and sits in the view's own toolbar; a boundary implies a `role="separator"` naming what it resizes; a toolbar implies a nested group holding more than one control; every tab implies an `id`, a real pane, and that pane naming it back; the frame implies its tokens are on the shell and not on the view. **Mutation-tested rather than assumed**: five regressions were introduced one at a time — drop the `.rux-app-view` class, drop `aria-expanded`, drop `data-rux-tabs`, unnest the flush group, move the frame token off the shell — and each failed exactly one test and only that one. **One defect found and recorded, not fixed: D2**, the gap between workspace and panel that this document forbids and the shared drawer mechanism creates, measured at 20px in the shipped app. **Deliberately not done:** no theme control was added — § Action Placement puts it in the profile menu or the Preferences surface reached from it, so a bare switch in the header would demonstrate a violation; both themes were verified by flipping `data-theme` instead. No rule changed, no name moved, no token value changed, and nothing in the application or the vendored consumers renders differently — the only page that renders differently is the example. Contract unchanged at **1.4.0**. |
