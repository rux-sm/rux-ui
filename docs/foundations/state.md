...
| 9 | Re-home the documents nav; then `menu.css` can drop `.is-active` | **[open]** | **The last `.is-active` in the portable layer, and it is a component decision rather than a state one.** `index.html:5390` is a `<nav aria-label="Forms">` wearing `.rux-menu` for its looks, whose item carries `.rux-menu__item is-active` **and** `aria-current="page"`. `menu.css` reads `.is-active` and `[aria-checked="true"]` but **not** `[aria-current]`, so the class is currently load-bearing there and cannot simply go. **The cheap fix is the wrong one.** Adding `[aria-current="page"]` beside `[aria-checked="true"]` is two lines and makes one selector serve **two different state semantics** — chosen-option and current-destination — which is one name for two concepts, the drift `naming.md` rule 2.4 forbids in modifiers, arriving in selectors instead. **The better design is to stop borrowing:** that nav is navigation, this system publishes navigation components that already read `[aria-current="page"]`, and composing it from one leaves `.rux-menu` with a single meaning and lets `.is-active` leave `menu.css` with no selector added. **What blocks it is that no existing navigation component fits an in-card vertical list** — `.rux-side-nav` is the overlay shell and `.rux-tabs` is horizontal — so this is either a small new block or a deliberate decision to accept `.rux-menu` for it and add the selector after all. That choice is `naming.md`'s territory as much as this document's, which is why it is its own step. |
