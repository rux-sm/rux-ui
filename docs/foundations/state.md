...
surface's own. Enforced: `tests/state-rules-contract.test.mjs`, step 10.)*
...
*(R3. Enforced: `tests/state-rules-contract.test.mjs`, step 10 — a drawer or attached panel
may carry `role="dialog"` only with `aria-modal`, which is what "honours it" means here.)*
...
Implemented in `rux-ui/js/drawer.js`; enforced by `tests/state-rules-contract.test.mjs`,
step 10.)*
...
disclosure channel, which is rule 2.1's problem wearing different clothes. Enforced:
`tests/state-rules-contract.test.mjs`, step 10 — all three clauses, each proven fallible.)*
...
| D5 | **Fixed (step 10)** — `tests/state-rules-contract.test.mjs` asserts all four, and enforcing them immediately found a live MUST NOT violation (see the step). ~~Rules 2.7–2.10 arrived unenforced.~~ |
