...
**Status** · 17 steps: **14 done · 3 open**
...
**Where to pick up.** The conformance program is done — the rules are stated, the roles read
the catalog, both gamuts ship, and rules 2.1, 2.8, 2.9 and 2.11 are executable. What is open is
**the cleanup**: three **Class C** removals (13, 14, 15), which stop and propose by
`CLAUDE.md`'s rule and carry their grep protocol in §5. **Q9 is answered** — step 16 retired `orange`, and the
trip palette is five colours; step 15's remaining question covers the other seven bases. **D15** is separate and needs a mechanism
...
| 13 | Retire the four `-fill-hover` roles | **[open] · Class C** | **The free one.** Step 9 moved the fills to 800, which is the step these named as *700's* hover, so they now resolve to the fill itself. **Grep protocol, 2026-08-22:** `--rux-danger-fill-hover`, `-warning-`, `-success-`, `-info-` — **css read 0** each, def 1 each, html 0, js 0, tests 0, docs only. Nothing anywhere reads one. **Removes 4 declarations, repoints nothing, renders identically.** Class C regardless, because a consumer on an older tag loses four published names — which is why it stops here rather than executing. |
| 14 | Retire the converged status and accent aliases | **[open] · Class C** | Step 5 and step 6 left these resolving to another name rather than removing them, because removal is Class C. **Grep protocol, 2026-08-22 (css reads):** `-strong` ×4 → **7** (`badges.css`, `trip-request.css`, tokens); `-vivid` ×3 → **5** (`team-chat.css`, tokens); `-bright` ×3 → **13** (`scheduler-app`, `driver-share`, `itinerary`, `trip-panel`); `--rux-accent-fill` → **4** (calendar now-line and today-fg, switch, slider). html 0, js 0, tests 0 for all fourteen names. **~29 reads to repoint before 14 declarations come out**, and each substitution renders identically because every one of these already resolves to its target. **One is not a rename and needs a decision:** `--rux-accent-fill`'s four consumers are mixed — the switch and slider are solid fills, but `--rux-calendar-today-fg` is *text* and `--rux-calendar-now-line-color` is a 1px line, and step 9 moved fills to the 800 step. Pointing all four at the accent would put a fill step on a text role. Split them first. |
| 15 | Retire the eight legacy hue bases (D5's chromatic half, D9, D10) | **[open] · Class C · turns on Q9** | **Grep protocol, 2026-08-22.** **Step 17 changed these counts materially.** `--rux-red`, `--rux-blue`, `--rux-cyan`, `--rux-yellow`, `--rux-green` and `--rux-pink` now have **0 reads** — six of the eight are free removals, because the trip palette that was their last consumer moved onto the catalog. `--rux-purple` keeps **4** (the tag fill in both themes, a priority dot, the driver priority colour). Original counts follow. `--rux-orange`, `-yellow`, `-cyan`, `-pink`, `-green`: **1 read each**, and every one is `--sched-trip-color-*` in `scheduler/css/tokens.css`. `--rux-purple`: **5** — the trip colour, `--rux-tag-purple` ×2, a priority dot, the driver priority colour. `--rux-gray`: **2** (D10) — `--rux-tag-default` and `scheduler-app.css:756`. `--rux-white`: **16**, `--rux-black`: **5**, `--rux-neutral`: **22** — all real, all in the off-catalog roles rule 1.1a names. html 0 and js 0 throughout; tests 3/2/16 and docs 57 for `--rux-neutral`, which are records and assertions rather than reads. **These are not dead vocabulary, which is what the earlier framing missed.** `orange`, `yellow` and `cyan` have exactly one consumer each and it is the scheduler's **trip-colour palette** — six distinguishable colours for colour-coding trips on the board. That is **Q9**, and it is a product question, not a conformance one. `--rux-neutral`, `-white` and `-black` cannot go at all until every rule-1.1a role has somewhere else to derive from. |
