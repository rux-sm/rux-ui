...
**Where to pick up.** No decision is open, and two steps remain: **7** makes rules 2.1, 2.8,
...
text on a status fill measures **3.44:1** in dark and 4.19:1 in light — below AA, and it is
the *catalog's own* 700 + 1000 pairing, so correcting it is a departure and needs a
decision. **D9, D10, D12** likewise stay open. §4 carries all of them.
...
measure 5.3–5.6:1 in light. *(Rux light-theme status text measures 1.19–1.85:1 — D6.)*
...
| **D14** | **Text on a status or accent fill is below AA, and the pairing is the catalog's own.** `--rux-danger-on-fill` (`red-1000`) on `--rux-danger-fill` (`red-700`) measures **3.44:1 in dark and 4.19:1 in light**; white on `--rux-accent` measures **4.43:1** in both. The destructive button and the primary button are the visible cases. This is not a mapping error — 700 is the catalog's high-contrast background and 1000 its text — so correcting it means departing from the catalog (a darker fill, or an 800 fill with a 1000 label), which is a decision rather than a fix. Rule 2.11 as written does not cover it: it scopes the AA floor to the 900/1000 steps against the two **backgrounds**, not against fills. | measured on `gallery.html` after step 5; before the step it was ~3.95:1, so the batch moved it slightly worse |
