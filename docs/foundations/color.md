...
**Where to pick up.** **Every step is done.** Seventeen steps, and what is left is two
defects rather than work: **D10** (`--rux-gray` has one consumer whose replacement is a
visual judgement about a scheduler marker) and **D15** (a fill's hover is `calc()` arithmetic
in two places, and the catalog publishes no 800-hover to adopt). Both need a decision, not a
migration.
