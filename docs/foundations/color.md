...
**Contract version: 4.1.0** · Stamped at the top so a downstream document can state the
version it conforms to. Authority without a version is only "whatever `main` says today,"
which is not control. See [`README.md`](README.md) §2.

**Status** · 36 steps: **36 done**
...
published — the P3 branch's further 140 shipped at step 11 and were removed at step 33 —
31 roles repointed, ~30 light-theme overrides deleted as redundant. **Every text
...
**The bookkeeping, recounted at step 30.** Twenty-two defects: eighteen resolved, D12 half,
D18, D19 and D22 still standing — D22 filed by step 30, when rendering a component beside its
own tokens showed `.rux-badge--solid` painting a band nobody had published, and widened by
step 31 to the subtle badge, where the same derived-instead-of-published tint costs contrast
rather than only vocabulary. Thirteen questions: eight carry an answer stamp (Q1–Q3, Q5,
Q7–Q10), Q13 is partly answered, Q11 has no answer and Q12 has half of one — and **Q4 and Q6
carry no stamp at all though the steps they block (6 and 4) are both done**, so they were
settled by execution and never written down. Flagged rather than repaired: writing an answer
nobody gave is worse than leaving one unstamped.

**Where to pick up.** **The engineering is closed; two decisions are not.** Every role that
has a catalog step reads it; the four that do not are named under rule 1.1a with their
reason. One gamut ships since step 33, every published text and fill pairing clears AA in
sRGB, and rules 2.1, 2.8, 2.9 and 2.11 are executable. The legacy Tier 0 is down to
...
**What is open, and it is one decision wearing two names.** **Q12's mark half** — B (a backed
chip) against C (an outlined glyph) — and `../trip-bar.md` rule 2.12's repoint onto the
step-24 band. Step 29 established they are the same decision: a saturated status mark clears
F2 *only* over the normalized band, never over the shipped 500/600 board. Step 30 widened
`../../status-mark-specimen.html` to all seven hues × all four status inks and the coupling
holds at full width — on the band B and C clear every one of 56 pairings in both themes
(worst 3.11 dark / 3.47 light); on the shipped board 28 of 56 fail. **B and C measure
identically** — same `surface-1` against the same fill — so the matrix cannot separate them
and the choice is an eyeball, which is why it is the owner's. Closing it closes **D18**, the
only violation rule 2.15 leaves standing. **Q11** (a gap source for categorical colour) is
open and blocks nothing. **D19** is the catalog's and stays open by design; **D12**'s
remaining half is the four roles that are not on a step.

*Counted by recounting, not by incrementing — the failure `../trip-bar.md` records against
itself and step 21 repaired here once already. The previous revision of this line read
"Nothing is open. Eighteen steps, sixteen defects, all closed" against a log of twenty-nine
steps and two open defects.*

**One gamut ships.** The hue scales were published twice between steps 11 and 33 — an sRGB
branch and a P3 branch behind `@media (color-gamut: p3)`. **Step 33 removed the P3 branch**;
every scale now has exactly one value per theme, and it is the sRGB one. The AA floor is
therefore evaluated on the values that ship, and the fills stay at 800 for the reason they
always did: `blue-700` with a white label measures 4.44 in sRGB.
...
its own token sheet; the values here are the ones that render. **The P3 half of this
measurement is a record, not a published branch** — step 11 shipped it, step 33 removed it.
It stays recorded because removing a branch does not unmeasure it, and anyone reinstating
one starts from these numbers rather than re-reading the catalog.
...
nobody chose: the declared chroma is a wish, and what renders is whatever the gamut boundary
allows at that lightness and hue. Every published step MUST be valid in sRGB.

*This was a two-branch rule between steps 11 and 33.* A P3 branch published a second, wider
value per hue step, and the rule then read "inside the gamut it is published for" with the
added requirement that every P3 value have an sRGB value behind it. **Step 33 removed that
branch**, and the rule collapses back to one clause.

*The nesting trap is kept because it is a live hazard for any future branch-style block.*
`color-gamut` is a **media** feature, so `@supports` cannot test it: `@supports (color-gamut:
p3)` is an invalid condition that silently evaluates false. That is how step 11 was first
written, and the block simply never applied — caught by measuring the rendered value in a
browser, not by any test, since the CSS is well-formed either way. A media query tests the
*display*; `@supports` tests the *syntax*.

*(D5's 30 out-of-gamut tokens are unaffected: they are the legacy hue bases at chroma 0.28,
and their fix is still the Class C removal.)*
...
of trusting it. Enforced by `tests/color-scales.test.mjs`. **A hue step carries one published value per
theme.** It carried two between steps 11 and 33 — the sRGB one, converted from the catalog's
HSL and recording it, and a P3 one the catalog authors in `oklch()`, measured rather than
converted and citing no HSL. Step 33 removed the second, so the provenance requirement is
uniform again: every step cites the measurement it came from.
...
**The floor is evaluated on the values that ship, and they are sRGB** (steps 11, 33). This
began as a worse-gamut clause: while a P3 branch existed, a pairing that cleared in P3 but not
in sRGB was not published, because the sRGB branch is what a display without P3 renders and it
cannot be the one that fails. `blue-700` with a white label measures **5.04 in P3 and 4.44 in
sRGB** — it would have passed on the machine most designers own and failed on a large share of
the machines the app runs on. **Step 33 removed the P3 branch, and the clause outlived it in
substance:** 4.44 is now simply *the* number, and it is still why the fills sit at 800 rather
than 700. The asymmetry the clause was written to catch is gone with the branch, but the
step it chose stands, and `tests/color-scales.test.mjs` pins it.
...
**2.15 A hue carries as many meanings as it likes, provided no two of them can occur
together.** Nothing in this document reserves a hue, and nothing should. Blue is
`--rux-accent`, `--rux-info` *and* the scheduler's confirmed status; amber is
`--rux-warning`, an accent variant *and* a trip category; green and red are each spent the
same way. **Reuse is not the defect.** A rule requiring one meaning per hue would cost the
palette a hue for every meaning it names, and with seven hues carrying more than seven
meanings that arithmetic does not close — which is why D17 sat open through five amendments
rather than being settled by whoever edited next.

**The test is simultaneity, not uniqueness.** Two meanings on one hue conform when a reader
is never asked to tell them apart. They conflict when both can render **in the same element,
the same comparison set, or the same instant** — because at that moment the hue is the only
channel carrying the distinction, and it is carrying two.

- **Conforms** — blue as `--rux-accent` on a button and blue as *confirmed* on a bar fill:
  different element class, never adjacent as a distinction a reader must make.
- **Conforms** — `--rux-accent-text` on the mini-calendar's current date and `blue-fill` on
  a confirmed bar: F3 wayfinding chrome against F1 data, two tiers apart.
- **Conflicts** — `--rux-success` as the mark *on a bar whose category fill is green*: same
  element, same instant, one hue asked to mean both. This is **D18**.

**Two consequences, and they are the whole point of the rule.**

**(a) A colliding meaning does not lose its hue; it gains a treatment.** Where two meanings
must co-occur, at least one MUST be distinguished by something other than hue — a fill
against a backed or outlined mark, a shape, a position. Colour then stops being the only
channel carrying the difference, which is where the accessibility floor has always pointed:
*"color should not be used as the only method of communicating what the data represents"*
(Cloudscape, cited under Q11). Note that this is also the only way F2 clears at all — §5
step 29 measured a saturated status mark as legible over the normalized band **only** as a
chip or a ring, never bare.

**(b) When a status and a category collide, the category's rendering yields.** The two are
not equally displaceable: a status hue is load-bearing vocabulary a reader learns once and
applies everywhere, while a category is an arbitrary tag whose only requirement is being
told apart from the other tags. A status MUST NOT be moved off its hue to resolve a
collision with a category.

**Known violation, recorded rather than implied:** D18 — the trip bar renders the status
900s on a category fill of the same hue. The rule does not fix it; `../trip-bar.md` rule
2.12's repoint plus a mark treatment does, and which mark is Q12's remaining half.
...
**The P3 branch — removed at step 33, recorded here as measured.** It restated every hue
step as `oklch()` under `@media (color-gamut: p3)` and left the greys as-is. Read and recorded for the 700s, the step the accent and the
...
| D22 | **A third band, published by a component, and nothing knew it was there.** `.rux-badge--solid` does not paint the role it is handed: it derives its own fill with `oklch(from var(--_badge-color) var(--rux-badge-solid-lightness) c h)` and its own label from `--rux-badge-solid-fg-lightness`. **That is rule 2.13's shape done independently** — one luminance per theme across every hue, which is exactly what a band is — but with its own number instead of the `--rux-{hue}-fill` / `-on-fill` pair §5 step 24 published for the purpose. **Measured 2026-08-25, sRGB branch: dark 78%, light 52%.** In dark that lands on **no step of any hue** — above every 900 (69.31–77.24) and below every 1000 (95.88–96.76) — so it sits in a gap rule 2.2 does not name. In light, 52% falls inside the 900 spread (47.06–53.52) without matching any hue exactly. **This is not a contrast failure and must not be reported as one:** every solid badge measures **7.53–9.78** against its own derived label in dark, well clear of 4.5. The defect is that the system now states the same rule twice, in two vocabularies, and only one of them is published — which is what `README.md` §One home per rule exists to prevent. **Found by rendering the component beside its own tokens** (step 30's provenance lines), not by any test: the badge resolved to real colours that cleared their floor, so nothing in the suite had reason to look. Closing it means either repointing the badge at the published band or recording it as a deliberate second band with its reason; both change what renders and neither is authorized here. **A second instance, found by step 31, which is what makes this a defect rather than a quirk of one modifier:** the *subtle* badge derives its tint the same way — `color-mix(in oklab, var(--_badge-color) 16%, var(--rux-surface-1))` — rather than reading the **100** step the scale already publishes and `--rux-{status}-subtle` already points at. **Here it costs contrast rather than only vocabulary.** On the derived tint the 900 text step measures **4.07–4.27 in light on five of seven hues**, under the 4.5 floor; on the published 100 step the same text measures **4.85–6.93, zero failures in either theme**. One cause — a component computing what the scale publishes — in two places, one of which fails. | measured 2026-08-25 on `../../status-mark-specimen.html`; `rux-ui/css/base/badges.css:75`, `tokens.css:1489,2319` |
...
| 30 | Answer Q10 — a hue's meaning is scoped by co-occurrence (rule 2.15; D17, D18) | **done · Class A** | **Executed 2026-08-25 under the owner's standing authorization to rewrite rules where it improves the palette. Rule text, an answer, and a widened specimen — no token, role or value moves and nothing re-renders.** **The answer is none of Q10's four branches.** All four freed a hue by amputation (`--rux-success` leaves green, `--rux-warning` leaves amber, the accent variants lose three, or the categorical palette shrinks to two); the owner's answer is that freeing a hue was never the requirement. **Rule 2.15: a hue carries any number of meanings provided no two can render in the same element, comparison set, or instant.** Uniqueness is not the test; simultaneity is. Two consequences are rule text: **(a)** a colliding meaning gains a *treatment*, not a new hue — which is where Q11's Cloudscape citation and the F2 floor arrive at the same answer — and **(b)** when a status and a category collide the **category yields**, because a status hue is vocabulary a reader learns once and a category is an arbitrary tag. **D17 closes**: the five hues carrying two or three assignments are conformant as such. **D18 stays open and is now a named violation** — it is the one pairing in the system that actually co-occurs — whose fix is `../trip-bar.md` rule 2.12's repoint plus Q12's mark, together, per step 29's coupling. **The repository had inferred this rule once already and never lifted it:** `scheduler/css/tokens.css:51` gives blue-as-confirmed its own name *"so it gets its own name here"*, citing D17 and then scoping past it — a rule solved at the point of use, which is the drift §One home per rule exists to catch. **`../../status-mark-specimen.html` rebuilt as a component gallery and widened to the full matrix**, because a scoping rule cannot be checked against a partial one, and because a swatch grid cannot answer the question 2.15 actually asks. **The widening:** both boards now cover **all seven catalog hues against all four status inks**, 28 pairings per treatment per board, with a rollup reporting the pairing closest to its floor. It was previously five hues and the danger ink alone, which understated the board twice — the shipped board carries *seven* fills at the category rung (`--sched-trip-bar-confirmed-tone` and `-unconfirmed-tone` are blue and red at 500/600), and `--rux-info` had never been rendered at all. **The reformat:** the page now leads with the components that spend a hue — a real `.sched-trip-bar` per hue carrying **every pending mark the app can raise at once**, read from `getPendingIndicators()`; solid and subtle badges; alerts; status text; the D20 count badge — each card stating its **function (F1–F5)** and **whether a status can co-occur on it**. That last line is the rule made legible: the gallery is a co-occurrence audit, and it shows that of every component here **only the trip bar puts two meanings in one element**. Board and treatment are global toggles, so a bar can be compared against itself across all four marks. The measurement matrix survives beneath it as the evidence this step cites. **Measured, sRGB branch, dark / light.** Shipped board: **A** 2.49 (danger on green) / 1.95 (info on red), **3 of 28** and **20 of 28** below floor; **B** and **C** 1.78 / 1.68, **28 of 56** both themes — the disc and the ring fail the fill because those fills sit near `surface-1`'s own lightness per theme; **D** 7.30 / 7.26, clears. Step-24 band: **A** 1.18 / 1.14, **22 of 28** and **28 of 28**; **B** and **C** **3.11 / 3.47, clears everywhere**; **D** 5.70 both themes, clears. Reproduces step 29's recorded 6.74 / 5.37 ink-on-disc and 3.11 disc-on-fill exactly, which is what validates the widened harness. **A correction the widening forced, recorded because it was mine to get wrong:** Q12 records step 23 concluding *"Dark mode needs no search — its bright 900s clear 3:1 on every fill including their own hue.
**Corrected by step 30: that generalisation is false.** Widening the specimen to all four
status inks found **danger on green at 2.49, danger on teal at 2.61** — both inside the set
step 23 swept — and `--rux-info`, never rendered until step 30, at **2.92 on green**. Dark is
*mostly* clear, not uniformly clear, and 3 of 28 pairings fail on the shipped board."* **That is false**, narrowly and inside the set step 23 swept: **danger on green measures 2.49 and danger on teal 2.61** in dark on the shipped board, and `--rux-info`, unmeasured until now, adds **2.92 on green**. Dark is *mostly* fine, not uniformly fine, and the rollup now states the count rather than a verdict so the next reader cannot repeat the generalisation. **Every specimen now names the token it paints from, and that is what found D22.** Each card carries a provenance line per colour — the role, the chain it walks, and the literal it lands on (`--rux-danger → --rux-danger-base → --rux-red-900 → #ff6166`), which makes rule 1.2's role-to-step hop visible instead of asserted. **Two things fell out of writing it.** First, a colour a component *derives* has no chain to show, which forced the distinction the page now draws between **given** (a token) and **derived via** (a governing token plus the measured result) — and that distinction is D22: `.rux-badge--solid` is handed `--rux-{hue}-fill` and paints something else. Second, those derived values must be read from the rendered element rather than computed from tokens, so the page gained a live measuring pass; **the first version of it silently measured nothing**, because a computed colour comes back as `oklch(0.78 0.186 258.469)` with lightness as a *fraction* where `tokens.css` writes a percentage, and the parser expected `rgb()`. It failed to a placeholder rather than to a wrong number, which is the only reason it was caught by eye. **A specimen bug found by the reformat and recorded because the next reader will hit it, per step 25's precedent:** treatment B wrapped the *entire* mark cluster in one 22px disc rather than one disc per glyph, so the icons overflowed it and rendered as a smear. **Every number on the page was correct while the page was wrong** — the ratio is disc-against-fill and does not depend on how many glyphs are inside the disc, so no measurement and no test could have caught it. It was visible the moment a real bar was rendered at real size, which is the argument for the gallery format in one sentence. **Deliberately did NOT:** pick the mark — B against C is the owner's, unchanged since steps 23, 24 and 29, and the widened matrix does not separate them (B and C measure identically, because the ring and the disc are the same `surface-1` against the same fill; they differ in how the glyph reads, which is an eyeball); repoint `../trip-bar.md` rule 2.12, which is that document's own step; touch D19, which is the catalog's and stays open; or renumber 2.2, which governs steps and is not the rule that was missing. **Needs an eyeball: nothing renders differently.** Contract 3.5.1 → **3.6.0**. |
| 31 | Measure the tint/text/line stack; 900 carries F3 and F4 together over the 100 step (D22) | **done · Class A** | **Executed 2026-08-25. Evidence and a specimen section — no token, role, rule or value moves and nothing re-renders.** **The question, from the owner:** can the **900** step carry the subtle badge's and the alert's *text* and *border*, not just their icon? **The answer is yes, and it is unconditional over the published 100 step — but only over it.** Rule 2.2 already calls 900 *secondary text and icons*; what was never measured is 900 as an **F4 line**, or either function over a tint a component computed for itself. **Measured, sRGB branch, all seven hues, both themes.** Over `--rux-{hue}-100`: text/tint **5.75–8.12 dark, 4.85–6.93 light** — clears the 4.5 F3 floor on every hue in both themes, **zero failures** — and the identical figure clears the 3.0 F4 floor as a line by a wide margin, with line/canvas 5.01–9.95. **One step serves both functions because the two floors differ and the 100 step is on the normalized part of the ladder** (rule 2.13's own evidence: 100–400 spread 1.0–3.7), so unlike the fill band it means the same lightness on every hue. **What does not work, and it is the same defect twice:** over the badge's *derived* 16% tint the 900 text measures **4.07–4.27 in light on blue, red, green, teal and pink** — five of seven under the floor — while the line, at a 3.0 floor, passes on the same numbers. **That is rule 2.14's function axis rendering its own verdict:** blue at 4.15 is a failing text and a passing line, one colour, two functions, two answers. **Also measured and worth recording because it reads as a defect and is not one:** the shipped subtle border, `color-mix(… 32%, surface-1)`, measures **1.15–1.77 against both of its neighbours** in both themes. `badges.css:56` states the intent — *the outline stays a hairline instead of out-shouting what it encloses* — and a decorative hairline around an already-labelled badge carries no information, so F4's floor does not bind it. Recorded so the next reader does not file it. **The alert's shipped form leaves `border-color: transparent` and its body text neutral** (`--rux-text-primary` at 14.33–16.83 on the tint); 900 on both clears, and the specimen renders them side by side. **`../../status-mark-specimen.html` gains a three-row comparison** — derived tint with the fill as text, derived tint with 900 as text, published 100 with 900 on both — because the failure only appears when the three sit together in light theme. **Deliberately did NOT:** repoint the badge or the alert. Both change what renders, the subtle badge's tint is D22's second instance and closing it is D22's step, and the alert's border is a visual decision the measurement does not make. **Deliberately did NOT** propose a categorical `--rux-{hue}-subtle` role: the 100 step already is one, and publishing an alias for a step a consumer can read is how the second band in D22 started. **Needs an eyeball: nothing renders differently.** Contract 3.6.0 → **3.6.1**. |
| 32 | Make the specimen's rungs selectable and sweep every combination (Q12) | **done · Class A** | **Executed 2026-08-25. A specimen capability and an exhaustive result — no token, role, rule or value moves and nothing re-renders.** **Why:** every open decision here is *which rung*, and a page offering two fixed boards can only confirm what whoever built it already believed. `../../status-mark-specimen.html` now takes the **fill rung** and the **ink rung** as free selections — all ten published steps on each axis, plus the step-24 band and the shipped per-theme board — driving the trip bars, the mark rack, the badges, the tint stack, the status text and the matrix together. **Where no label is published, the page picks one by measurement and says so:** only the band ships a per-hue `-on-fill`, so at any other rung the label is chosen white-or-near-black by contrast and annotated `label·measured 5.02`, which is rule 2.11's own method made interactive instead of asserted. **The sweep is the point.** One button measures **every fill rung against every ink rung — 132 pairs, each across 7 hues × 4 status inks in both themes** — and lists only what clears in both. **The result converts an argument into a proof.** Step 29 and step 31 could say *we did not find a rung that rescues bare ink in light*; the sweep says **there is none on the band**: of 23 combinations that clear treatment A in both themes, **not one uses `--rux-{hue}-fill`**. The band's fill sits at one luminance (0.115, amber 0.2524 — the gamut exception), so clearing 3:1 against the whole set demands an ink at luminance ≥0.856 or ≤0.005, and **no published step is that extreme in both themes** — dark's 1000 manages it (0/28, worst 3.05), light's 1000 does not (24/28, worst 2.15). **What does clear with a saturated fill is `shipped board + 1000 ink` (6.41 dark / 5.02 light)**, which independently reproduces Q12's own finding — *the only light-mode pairing that clears everywhere is the hue's 1000 ink* — and lands back on the desaturation the owner declined there (1000's chroma is **0.017–0.032 dark, 0.045–0.084 light**, against the 900s' 0.125–0.197). **So the choice on the band is a treatment, not a rung**, and the sweep now demonstrates that rather than asserting it: run it on **C** and 9 combinations clear, the band among them at **3.11 / 3.47** with the 900's chroma intact. **A fifth treatment was added and is not a recommendation:** `A/C`, bare in dark and ringed in light, because the failure is per-theme — the band's fill luminance is theme-invariant by design while the status ink is not, so in dark the 900s sit at L 70–77 *above* the band's 47–64 and in light at L 52–55, *inside* it. Its rollup still reports dark's **22 of 28** honestly, which is the useful part: **A does not clear in dark either** (2.16–3.02, short by 0.1–0.9, and 1.18–1.65 on the amber fill), so a per-theme mark fixes half a problem. **Recorded because it is easy to misread:** A's dark numbers are *uniform* — every ink lands within 0.02 across the six non-amber fills — which is step 24's band working exactly as designed and is why the treatment reads as coherent. Coherent and below the floor are not exclusive. **Deliberately did NOT:** pick a mark or a rung; touch any token; or treat the 23 clearing combinations as candidates — most pair a tint rung with a text rung and describe a badge, not a saturated category bar. **Extended the same day, at the owner's ask: the selection is per element, not global.** One shared pair of rungs can only show every element agreeing, and the question in front of D18 is whether a *mark* and a *fill* should sit on different rungs — which a single setting cannot express. Each element — trip bar, mark rack, solid badge, tint stack, status text, count badge, matrix — now carries its own strip: **fill rung, label, ink rung, treatment**, plus a row of hue buttons that are themselves swatches at the element's own fill rung. The bar at the top became *set all*, a starting point that each strip overrides. **The label gained its own axis**, because a label is not always a rung: `auto` (rule 2.11's method — whichever of white or near-black measures better, annotated with the figure), `published` (the band's `-on-fill`, which no other rung has), the two neutral inks by name, or any of the ten steps. Where a rung publishes no label the page says `label·measured 5.02` rather than presenting a guess as a fact. **`gray` is selectable alongside the seven hues** — it is a published scale, it has no band step, and letting a rung be chosen for it is the point. The sweep now targets a named element and honours its hue selection, so it answers *for this component* rather than for an average. **Needs an eyeball: nothing renders differently.** Contract 3.6.1 → **3.6.3**. |
| 33 | Remove the P3 branch (reverses step 11) | **done · Class B** | **Executed 2026-08-25 on the owner's instruction.** Deletes the whole `@media (color-gamut: p3) { @supports (color: oklch(0% 0 0)) }` block — **140 declarations**, seven hue scales, ten steps, both themes — plus its 24-line header comment. `tokens.css` 2577 → **2388 lines**; brace balance verified at depth 0. Every hue step now has exactly one published value per theme, and it is the sRGB one. **Class B, and it is the mirror image of step 11's.** An sRGB display never matched the media query, so nothing it renders changes — **0 of the values an sRGB display resolves moved**. A P3 display re-renders every chromatic colour in the system: each of the 140 steps stops resolving to its P3 value and falls through to the sRGB declaration underneath it. Against the values at HEAD that is the *narrowing* direction — `--rux-blue-700` `oklch(57.61% 0.2321 258.23)` → `oklch(57.91% 0.2141 257.97)`, and equivalently across all 140. **Stated against HEAD deliberately, because the fall-through target is itself in flux:** the owner's palette rebuild is changing the sRGB declarations for blue, red and amber in the same working tree, so on a P3 display those three scales move for two independent reasons and only one of them is this step's. Isolating the pair this way is the only way the number means anything. Greys, the alpha scale and the two backgrounds were never in the branch and are untouched. **Contrast moves in the safe direction, which is why this needed no re-measurement to be sound.** Rule 2.11 already evaluated every pairing in the worse gamut — sRGB — so every published pairing was already certified against exactly the values that now ship. Removing the wide branch removes only the *better* number from each pair; it cannot lower a ratio below a floor that was set by the other one. `blue-700` with a white label was **5.04 in P3 and 4.44 in sRGB**; 4.44 is now the only number, and it is unchanged. **Rules amended:** **2.9** collapses from two branches to one and keeps the `@supports`/media-feature trap as a live hazard for the next branch-style block; **2.10** drops "a hue step carries two published values"; **2.11**'s worse-gamut clause is rewritten — the asymmetry it was written to catch is gone with the branch, but the 800 fill step it chose stands and is now pinned structurally. §2's rollup and §3.1's source note follow. **§3.1's and §4's P3 measurements are kept, not deleted** — removing a branch does not unmeasure it, and anyone reinstating one starts from those numbers. **Enforcement, reduced honestly rather than repointed.** Three suites sliced the file at the P3 marker. Two guarded the slice (`P3_AT > 0 ? … : tokens`) and degraded silently; `fill-band-contract.test.mjs` did **not** — its bare `CSS.slice(0, CSS.indexOf(…))` would have become `slice(0, -1)` and quietly dropped the file's last character, which is the kind of green that means nothing. All three now read the whole file. **Two tests were deleted rather than adapted**, because each asserted a property of a branch that no longer exists: `color-scales`'s "the P3 branch covers every hue step in both themes, and only hues" (it opens `assert.ok(P3_BLOCK)` and fails outright), and `color-contract`'s "every P3-branch step resolves inside P3, and has an sRGB value behind it" — which did **not** fail, and that is worse: with an empty branch it iterated an empty map and passed vacuously. **One test was kept and renamed**: the surviving half of the P3 fill test asserted that the published fills read the 800 step, which was never really about P3, and is now "every published fill reads the 800 step (rule 2.11)". Suite 434 → **432 tests**. **Verified: zero regressions attributable to this step.** The suite failed 6 before and 6 after, and the two sets are identical — all six are the owner's in-flight palette rebuild (rule 2.9 gamut, rule 2.11 fills, theme-invariance, provenance comments, the browser fixture, and the value snapshot), none of them this step's. The transient 7th failure was the deleted `assert.ok(P3_BLOCK)` test, resolved by deleting it. **Deliberately NOT done, and this is the important half.** The **light theme was not touched** and the **token snapshot was not regenerated** — the owner scoped this session to dark mode, and the palette rebuild in the same working tree is unfinished, so regenerating now would bake half-built values into the committed baseline and destroy the only record of what the palette used to be. The snapshot is the palette step's to regenerate, not this one's. Also not done: **no re-measurement of published pairings**, for the reason argued above — the floor was already set in sRGB, so there is nothing to re-derive; and **no change to any fill, role or step assignment**, which stay exactly where step 9 put them. **Needs an eyeball: every chromatic surface, on a P3 display, dark and light.** Nothing changes on an sRGB display, so the check is specifically that the narrowed colours still read as intended on wide-gamut hardware — buttons, badges, alerts, status text and the trip bar. Not yet performed. Contract 3.6.3 → **3.7.0**. |
| 34 | The three-step model — every hue-derived role lands on 200, 400 or 900 | **done · Class B + Class A** | **Executed 2026-08-25 on the owner's instruction, dark theme only.** The owner rebuilt the seven hue scales as an even ramp — L 10→90 in tens, C 0.18 below the top (amber 0.14), C 0.24 at 900 — and that changes what a step *is*. Geist's scales were lightness-normalized nowhere in the fill band (spread **28.8** at 800, D19's root cause); the even ramp's spread is **0.0**, so a fill band can be chosen once for all seven hues instead of per hue per theme. **The roles move onto three steps: 200 tint, 400 fill, 900 ink.** `--rux-{danger,warning,success,info}-subtle` 100 → **200**; `--rux-{danger,warning,success,info}-fill` 800 → **400**; `--rux-button-accent-background` `accent-800` → `accent-400`; `--rux-accent` `accent-700` → **`accent-900`**; `--rux-accent-subtle` `accent-100` → `accent-200`; `--rux-link-fg-hover` `accent-1000` → `accent-900`; and the scheduler's seven trip tones 500 → **400**. **Class A rider:** `--rux-accent-200` and `--rux-accent-400` are published as new mirrors, in `:root` and in all three `[data-rux-accent]` blocks — 8 declarations. Nothing is removed; 100/700/800/1000 stay published and now unread. **Before → after, resolved, dark:** `danger-fill` `red-800` `#ff6c6d` → `red-400` `#90000c`, its label **1.95 → 9.58**; `info-fill` **1.86 → 9.56**; `warning-fill` **1.92 → 9.42**; `success-fill` **1.72 → 8.07**. `--rux-accent` `blue-700` → `blue-900`, **12.41** on the canvas. Trip tones `#0057e6`-class 500s → `#003ca6`-class 400s. **`--rux-warning-on-fill` and `--rux-success-on-fill` flip from `-inverse` to `--rux-fg-on-fill`** — at 400 every fill is dark, so one white label serves all four, which is the first time this document has had a uniform on-fill and is the practical answer to D19 from the Rux side. **Step 1000 came off L100 in the same pass.** It was `oklch(100% 0.24 h)`; at L100 nothing but white is in gamut, so the clip rotated hue by up to **62°** and collapsed four scales into two colours — `blue-1000` `#99fcff` and `teal-1000` `#00ffff` both cyan, `purple-1000` `#ffceff` and `pink-1000` `#ffb0ff` both pale magenta — and it shipped, because `--rux-accent-1000` backs `--rux-link-fg-hover`. Now `oklch(95% 0.04 h)`: drift **15°**, closest pair 32°, no collisions. L92 measured better (6°) and was rejected for sitting 2 lightness points from 900, which stops the step earning its place. **THE MEASUREMENT METHOD CHANGED MID-STEP, AND THE EARLIER NUMBERS IN THIS SESSION WERE WRONG.** The analysis began by modelling CSS Color 4 gamut mapping — reduce chroma, hold L and H. **Browsers clip.** Rasterised through a canvas, `oklch(90% 0.24 24)` paints `#ff908d`, where the chroma-reduction model predicted `#ffd1cd`. Every figure derived from the model for an out-of-gamut value was wrong, and `status-mark-specimen.html`'s own arithmetic — which clips, and was briefly reported here as buggy for doing so — was right. Two conclusions were reversed by this: that the fill band should be 500 (it is **400**; 500's white label measures 4.44, under the floor), and that the 900 inks collapse to near-grey and become mutually indistinguishable (they paint `#ff908d` / `#ffca58` / `#4bff76` / `#5ddeff`). **Everything above is canvas-rasterised, not modelled.** **Enforcement repointed, not relaxed.** `color-scales`'s rule 1.1 map takes the three steps; its fill test is renamed to the 400 band; `trip-bar-color`'s tone assertions take 400 in dark. Three tests failed on the move and had their expectations updated — the ratchet working, as in step 9. 432 tests, 426 pass. **THE LIGHT THEME IS BROKEN BY THIS, DELIBERATELY, AND IT IS THE HALF THAT MATTERS.** These roles are declared once and shared by both themes — a rule with its own test — so a repoint reaches light, whose scales are still Geist's. Light-400 is L91%, a near-white tint, so all four fills now measure **1.25–1.31** against their white label, and `--rux-button-accent-background` **1.29**. Dark measures 8.07–9.58 on the same tokens. Only steps 700 and 800 are theme-invariant in Geist (all 7 hues, exact), which is precisely why the fills sat at 800 and why nothing else could be shared; the even ramp destroys that invariance and light cannot be repaired without rebuilding its scales. **This was stated before executing and accepted; the palette is scoped to dark for now.** **Deliberately NOT done:** the Class C sweep — 100/700/800/1000 and `--rux-accent-{100,700,800,1000}` stay published and unread, per the owner's "three is a good starting point, adjust later"; the token snapshot, still unregenerated for step 33's reason; `--rux-{hue}-fill`, the step-24 band, whose seven literals the rebuild never touched and which is required theme-invariant by its own test; and `--rux-accent-hover`, left on 800 rather than retired to the composited overlay, which is a component change and not a token repoint. **Rule 2.2 is now describing ten purposes where three are used** — recorded here rather than amended, because the owner has said the model may still move. **Needs an eyeball: everything chromatic, dark, and the light theme is known-broken so do not judge it.** Nothing has been viewed at width or in the application shell. Contract 3.7.0 → **3.8.0**. |
| 35 | Remove the step-24 fill band (reverses step 24) | **done · Class C** | **Executed 2026-08-25 under explicit authorization, after the proposal in this row's grep protocol.** Removes `--rux-{hue}-fill` ×7 and `--rux-{hue}-on-fill` ×7 in **both themes — 28 declarations** — plus the band's 24-line header comment, and retires `tests/fill-band-contract.test.mjs` (5 tests). Suite 432 → **427**. **GREP PROTOCOL, PRE-EDIT:** `index.html` **0** · `js/` **0** · `scheduler/css` **0** · component CSS **0** · specimens **2 files** · `tests/` 28 (its own contract test plus the value snapshot) · `docs/` 0. **The band was never read by a component in any commit** — `git log -S` across all history over `rux-ui/css`, `scheduler/css`, `js/` and `index.html` returns nothing. **Both vendored consumers were checked on disk** — `infor-ln-docs` and `rux-infor-ln-guides` — and neither reads it. **POST-EDIT:** css 0 · js 0 · html 0 · docs 0; the only surviving mentions are this log and the two specimens' prose. **Why it goes, and it is not that it was wrong.** Step 24 built it to answer D19: Geist's hue scales are not lightness-normalized in the fill band — spread **28.8** points at 800 — so no single label serves seven hues, and the band solved every hue to one luminance per theme, getting the spread to **16.5**. Step 34's even ramp gets it to **0.0** by construction, which answers D19 at the scale rather than beside it. The band became a hand-tuned Geist-era outlier in a system with no other Geist values left in dark: `--rux-amber-fill` was L63.7% where every other fill is now L40%. **Measured against the model that replaced it:** a bare 900 status ink bottoms out at **2.53 on `amber-fill`** against a 3.0 floor and its white label at **3.47**, where the 400 step clears both at 3.35 and 7.36. **The consumer it was written for never took it** — `--sched-trip-color-*` has always read a numbered step (500, now 400), never the band. **Class C and a MAJOR contract bump, 3.8.0 → 4.0.0.** Fourteen published names are gone. No consumer migration was needed, which is why this is the cheapest Class C this document has run, but the version has to say a name was removed regardless — a downstream pinned to 3.x conforms to a vocabulary that no longer exists. **A specimen was broken by the removal and the fix is recorded because it is the interesting part.** `status-mark-specimen.html` threw on load: its `paint()` helper called `mixOver()` on `TOK.get("rux-{hue}-fill")`, which now returns null, and the `if (!bg || !ink || !line) continue` guard sat *downstream* of the throw. Three sites needed it: the `mix-fill` tint row (deleted — it referenced the band), the matrix's reference rung (`others` compared against the band; now compares against the shipped board rung), and the fill/label dropdowns. **`paint()` is now fail-soft** — a spec naming an unresolvable token returns null and is skipped rather than throwing — which is the general fix, not the specific one. Verified by a clean load: 8 sections, 7 bars, 21 badges, `render()` returns. The tint row's own pair moved 100 → 200 to match step 34 while it was open. **Deliberately NOT done:** `trip-bar-fill-band-specimen.html` is left in place though its entire subject is gone — it is the exploration record for step 24 and deleting it would erase why the band existed; it is **now orphaned and will not resolve its tokens**, which is recorded here rather than hidden. The value snapshot stays unregenerated (step 33's reason, now compounded). D19 is **not** closed by this step: it is the catalog's defect, §3.1 records the catalog as measured, and step 34 is what answered it on the Rux side. **Needs an eyeball: nothing — no component ever read these tokens, so nothing renders differently.** That is the one claim in this row a test can verify and does: 427 tests, 421 pass, and the 6 failures are step 34's known set. Contract 3.8.0 → **4.0.0**. |
| 36 | The light theme joins the three-step model | **done · Class B** | **Executed 2026-08-26. The owner started this — blue, red and amber on a descending L 95→10 ramp at C 0.18 — and asked for it finished.** All seven light hue scales are now `L 95, 90, 65, 40, 36, 32, 28, 24, 20, 12` at the hue's own chroma (0.18; **amber 0.14**, matching dark, because amber's gamut ceiling is lower than the rest). 70 declarations. Green, teal, purple and pink were still Geist's and are the last Geist values to leave the system. **Only three steps are load-bearing and each was measured, not chosen:** **200 = L90**, a tint on a white canvas (wash **1.27–1.82**, with its 900 ink at **8.82–13.11** on it); **400 = L40**, a fill carrying a white label (**7.36–10.15**); **900 = L20**, ink on a white canvas (**15.92–17.68**). The other seven interpolate and have no consumer. **THE OWNER'S RAMP WAS RIGHT EXCEPT AT ONE STEP, AND THE MEASUREMENT IS WHY.** A linear descending mirror puts 400 at **L70**, where a white label measures **2.31–2.93** — a clean fail. The sweep: L70 2.31, L60 3.30, L55 3.99, **L50 4.83 (first to clear)**, L40 7.36. Light cannot be a linear mirror of dark, because the fill must stay dark enough for a label that is white in *both* themes. **400 IS NOW THEME-INVARIANT, AND THE INVARIANCE IS DERIVED RATHER THAN OBSERVED.** Light-400 at L40 with dark's chroma paints the identical bytes — `#003ca6 #90000c #753300 #005e00 #00624d #601b92 #8a0047` — and the measured contrast is the same either side of the toggle: danger 9.58, info 9.56, warning 9.42, success 8.07. Under Geist, 700 and 800 were theme-invariant as a *measurement of the catalog*, carried with one exception (red-800, one code value). Here it follows from the rule: every fill carries `--rux-fg-on-fill`, a single theme-invariant white literal, so a fill that changed with the canvas would need two labels and there is only one. **`tests/color-scales.test.mjs` is repointed from 700/800 to 400 and its exception list is deleted** — an exception existed because §3.1 recorded what the catalog measured and flattening it would have been choosing; these values *are* chosen, so an exception would mean a mistake. **Rule 2.11 goes green.** The four light fills measured 1.25–1.31 against their white label after step 34 and now measure 8.07–9.58. That was the whole of step 34's known-broken half. Suite 427, **423 pass, 4 fail** — down from 6: the browser fixture (Geist-era values), rule 2.9's gamut check (declared chroma exceeds sRGB, by decision), the provenance comments, and the deliberately-unregenerated snapshot. **A FINDING THE LIGHT THEME PRODUCES IMMEDIATELY, recorded because the next reader will hit it:** treatment **A · bare 900 ink fails in light on all 28 combinations** (worst 1.58, danger on purple), where it clears in dark at 3.35. It is structural, not a tuning problem: in dark the 900 ink is L90 on an L40 fill and reads; in light the same shared role is L20 on the same L40 fill, and two dark things collide. The mark cannot be 900 in one theme and 100 in the other because the role is declared once. **`A/C` — bare in dark, ringed in light — clears at 7.36 and is exactly the published treatment for this**, which is step 29's per-theme argument arriving again from the other side. B, C and D also clear at 7.36. The choice stays the owner's and Q12 stays open. **Two PRE-EXISTING specimen bugs fixed while verifying, both confirmed present in HEAD and neither introduced here.** `status-mark-specimen.html` had **two identical `#theme` click listeners**, so every click toggled twice and the theme button had never worked — the light half of that page was unreachable by its own control, which is why nothing had been eyeballed in light. And its token slice still read `css.slice(0, css.indexOf("@media (color-gamut: p3)"))`, which became `slice(0, -1)` when step 33 removed that block; harmless only because the file's last character is a newline. Both fixed; the parser now agrees byte-for-byte with the browser's computed value for a light token. **Deliberately NOT done:** the unused steps stay published (the Class C sweep is still pending); the snapshot stays unregenerated; `--sched-trip-bar-notes-fg` still reads `--rux-amber-1000` in light while dark reads 900, and the specimen's board option still hardcodes `dark ? 400 : 600` against the scheduler's light 600 — both are the scheduler's to move and neither is wrong today. **Needs an eyeball: light, everywhere.** The gallery renders and was screenshotted; nothing has been seen in the application shell or at width. Contract 4.0.0 → **4.1.0**. |
---
...
**Q10 — Does a hue carry one meaning? — ANSWERED (step 30): no, and the question was the
wrong shape.** By the owner, 2026-08-25, and it is **none of the four branches offered** —
which is worth recording, because all four were framed as ways to *free* a hue and the
owner's answer was that freeing a hue was never the requirement. The answer is **scoped**:
a hue may carry any number of meanings provided no two of them can render in the same
element, comparison set, or instant. Written as rule **2.15**.

**Why this is the cheap answer where every offered branch was expensive.** The question
assumed global uniqueness or nothing, so every branch amputated something — `--rux-success`
leaves green, `--rux-warning` leaves amber, the accent variants lose purple/green/amber, or
the categorical palette shrinks to teal and pink. Scoping converts a palette amputation into
a **treatment change on one component**: nothing gives up a hue, and D18 becomes a mark
problem rather than a palette problem. The owner's own examples are the argument — an accent
button and a confirmed bar both blue, the mini-calendar's current date on the accent — and
the observation that reuse there reads as *more* coherent, not less, because one idea wearing
three hats is not three ideas fighting.

**The repository had already answered it once, in practice, and nobody had noticed.**
`scheduler/css/tokens.css:51` gives blue-as-confirmed its own name rather than chasing the
meaning off blue: *"Blue-as-confirmed is the scheduler's own meaning (color.md D17), so it
gets its own name here."* That comment names D17 and then solves it by scoping — a rule
inferred correctly at the point of use and never lifted into the foundation, which is the
drift `README.md` §One home per rule exists to catch.

**The asymmetry survives the answer and is now rule text (2.15b).** A *status* role and a
*categorical label* are not equally displaceable — a status hue is load-bearing vocabulary a
user learns once, while a category is an arbitrary tag whose only requirement is being told
apart — so when the two collide, the category's rendering yields.

**Deliberately not answered here:** which mark D18 takes. That is Q12's remaining half, it
changes what every pending bar looks like, and step 29 established it is the same decision as
`../trip-bar.md` rule 2.12's repoint.
...
band.** `../../status-mark-specimen.html` — a component gallery since step 30, leading with a
real trip bar per hue under each treatment — renders four treatments — today's bare 900 ink, a
