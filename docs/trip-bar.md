...
status-on-fill standard (`color.md` Q12), the hue-meaning question (`color.md` Q10), and
the orphaned rung (`typography.md`).
...
yet governs a status mark's contrast on a saturated category fill.** In dark theme the marks
clear 3:1 on every fill including their own hue; in light theme they fail (1.94–3.33), which
is **D19 — an accepted defect, the owner's call after a measured seven-pairing sweep**
(`color.md` step 23 records the sweep; the standard itself remains `color.md` Q12's).
...
| Row type | **`--rux-text-label-12-*` whole, SM on `label-14` whole** (step 5) | conformant (2.6) ✓ |
...
| **D5** | **Fixed (steps 6 and 5, in that order).** ~~The default tier reads Tier 0 primitives while the XXS tier reads the Label role. Same pixels, one of them conformant.~~ Step 6 retired the (conformant) XXS tier; step 5 moved the default **and SM** tiers onto their Label roles whole. | 2.6 | `tokens.css:116-117` vs `layout/scheduler.css:162-164` |
...
| **D7** | **Fixed (step 7).** ~~Driver-role status is painted by `iconEl.style.color`.~~ The class-based mechanism exists and is **dead**: `driverStateClass()` is defined and never called, and its three `.sched-trip-bar__driver-dot*` rules have no emitter. | 2.9 | `trip-bar.js:459,1106`; `trip-bar.css:1088-1101` |
| **D8** | **Fixed (step 8, Q3 answered: no panel).** ~~`--sched-trip-bar-meta-bg` resolves to **0% alpha in both themes** and `-meta-shadow` to `none`, so the time and driver rows paint a background and a shadow that do not exist.~~ The comment beside them — *"recessed rim, same as input fields"* — describes a treatment that has not rendered for some time. | — | `tokens.css:80,113,213`; read at `trip-bar.css:965-968,1033-1035` |
...
| **D10** | **Fixed (step 8).** ~~Three dead declarations: an empty `@container (max-width: 24rem) .sched-trip-bar__time {}` rule; `stroke-width: 2` on `__pending-icon`, an SVG property on an icon-font glyph; and `padding-right: var(--rux-space-2)` on the same element, doubling the `gap` its row already sets.~~ | — | `trip-bar.css:1240-1242, 847-851` |
...
| **D19** | **Accepted by the owner, 2026-08-24 — open by choice, not by neglect.** The pending and driver status icons fail the 3:1 non-text floor on every **light-theme** fill (1.94–3.33; the common blue and red bars are the worst cases). The fix is measured and known — the hue's `1000` ink clears 5.0–9.8 everywhere — and the owner declined it after seeing all seven candidate pairings rendered side by side: the 1000s read too desaturated, and every alternative pairing fails outright (`color.md` step 23 records the sweep and the structural reason: the light branch has one dark-ink band). Dark mode is unaffected — its bright 900s clear on every fill including their own hue. **Reopen this** if light-mode usage grows, or when `color.md` Q12 produces a mark that is both saturated and legible (a backed chip or outlined glyph — a shape change, not a rung change). | 2.1 | measured 2026-08-24, sRGB branch; seven pairings sampled live, both themes |
...
| 5 | Row typography onto the Label role (D5) | **done · Class A** | **Executed 2026-08-24, wider than written by two findings.** The default tier now reads `--rux-text-label-12-{size,line-height,tracking}`; **SM reads `label-14` whole** — it had the same primitive-reading defect D5 named in the default, and fixing one tier while leaving the other would have moved the defect, not closed it. **Second finding: step 6 had silently orphaned `--sched-trip-bar-row-tracking`** — its only declaration lived in the XXS block that step deleted, so both readers were on their fallbacks; each tier now declares it with its pair, because an axis adopted separately is the defect rule 2.6 exists to prevent. **Proven invisible, not argued:** computed `font-size/line-height/letter-spacing` snapshotted on live rows before and after — byte-identical in both tiers (12/16/0 and 14/20/0), default bar height exactly 128px both times; a 0.008px SM bar-height delta is lane-layout float jitter, and the tracked axes did not move. `label-12` and `label-14` resolve to exactly the primitives they replace, which is what makes this Class A. Enforced: a new assertion in `tests/trip-bar-size.test.mjs` requires every tier's three row axes to name a `--rux-text-label-*` role, so a primitive can't creep back. 424/424. |
...
| 7 | Driver status onto a modifier class; delete the dead mechanism (D7) | **done · Class A** | **Executed 2026-08-24.** `applyDriverStatus` now adds `__driver-role-icon--{danger,warning,success}`; the classes read the same `--sched-trip-bar-*-icon` tokens the pending icons use, so both kinds of status mark on this surface are one system — and they inherit that system's open light-theme contrast problem (rule 2.1's note, `color.md` Q12), which this step neither worsens nor fixes. `driverStateClass()` and the three `__driver-dot*` rules deleted in the same change, as specified. **Found in passing:** the inline paint also set `--_icon-fill` to `--rux-icon-fill-selected` — a no-op, since both it and the bar's own icon fill resolve to 1; dropped rather than ported. **Verified live:** 12 of 17 role icons on the board carry tone classes, zero inline colours, and the sample computes the identical green-900 the inline `var(--rux-success)` painted — no rendering change. Enforced in `tests/trip-bar-color.test.mjs`: the emitter exists, `.style.color` writes are forbidden, and both halves of the dead mechanism stay deleted (declarations only — comments may name the history). **Deliberately did not** reuse the dead `driver-dot` names: they describe a dot the current design does not draw. |
| 8 | Delete the declarations that paint nothing (D8, D10) | **done · Class A, with a scoped Class C rider** | **Executed 2026-08-24, after asking Q3 first as specified — the owner answered: no recessed panel, delete it all.** Removed: the entire meta-panel family — `-meta-bg` (both themes), `-meta-shadow`, `-meta-radius` and the paired corner-roundings on `__time`/`__drivers`, and **`-meta-fg`** (the Class C rider: D16's famous token, single in-repo reader, not in any vendored surface; `__time` now reads `--rux-fg-on-accent` on-element, where the bar's own override makes it theme-correct — same resolved value, one less indirection). Also removed per D10: the empty 24rem container rule, `stroke-width: 2`, and the doubled `__pending-icon` padding — **the one visible change**: pending icons sit at their row's own gap now instead of gap + 8px, which is the defect closing rather than a side effect. Also swept: step 18's parked `-tail-hover-opacity`/`-tail-pressed-opacity` (both themes). **Verified live:** `__time`/`__drivers` compute transparent/flat/no-shadow — identical to what the zeroed tokens painted — with the label unchanged; pending-icon padding 0. **Deliberately did not** touch `-bg-lightness`/`-bg-opacity` (live driver-grid consumer, the naming defect is naming.md's), their unread `calc()` derivatives, or `-chroma-scale` — the recipe family's retirement is one decision and it belongs with the driver-grid rename, not split across a cleanup. Tests: the D16 assertions repointed at the ring token and a stays-deleted guard added; 424/424. |
...
**Q3 — Were the time and driver rows meant to be a recessed panel? — ANSWERED (2026-08-24):
no.** The owner chose deletion over restoration when step 8 asked: the treatment had rendered
as nothing for a long time, nobody missed it, and the bar's whole §2.3 posture is that it
separates by fill alone. The panel family — tokens, corner-roundings and `-meta-fg` — went
with the answer. *Original question kept below, because the "designed, built, then zeroed"
archaeology is why the step asked instead of assuming.*
