...
| 5 | ~~Namespace migration `.rux-*` → `.sched-*` (§3)~~ **done — closed 2026-08-22** | **Status correction (2026-08-22).** This row read *blocks done; elements remain — 585 occurrences across 46 files* for five days after it stopped being true, and its follow-up cited **51** domain-named tokens when the real figure had fallen to 17. **Step 13 paid the element half** (~2,400 occurrences across 61 files) and never came back to update this row, so anyone reading the ledger in order planned work that was already done — the one place a stale number does real damage, since this document governs how public names change. **What actually remained** was the domain-token follow-up, and **entry 26 closed it**: the last 17, all `--rux-calendar-*`. The redundant `scope` marker (`.sched-scope-trip` → `.sched-trip`) is the only deferral left and stays a separate decision. Original note follows. 585 occurrences across 46 files. Two follow-ups deliberately deferred, each its own decision: the 51 domain-named `--rux-*` **tokens** (§4.7) still carry the old prefix, and the redundant `scope` marker (`.sched-scope-trip` → `.sched-trip`) was left in place rather than combining two transformations in one wide rename. **Status correction (2026-08-17):** the migration renamed *block* classes, but ~28 families of **element** classes still carry the old prefix in the app layer — `.rux-bus-picker__*`, `.rux-trip-bar__head/__tail`, `.rux-tasks__*`, `.rux-team-chat__*`, `.rux-scope-*__*`, and peers — consistently in both CSS and markup, so nothing renders wrong, but the §3 "prefix is truth" rule is not yet fully realized. Completing the element rename is an open follow-up of this step. |
...
| 23 | ~~Retire the deprecated `.rux-app-shell` aliases~~ **done (2026-08-22)** | Class C, executed on the owner's approval; `docs/foundations/naming.md` step 9(a). **This closes the condition entry 10 wrote and entry 16 relied on.** Entry 10 kept `.rux-app-shell`/`__workspace`/`__panel` published *"until the vendored consumers sync their `rux-ui/` copy"*, and entry 16 confirmed at commit `157b427` that the portal and `guide_runner` were still using them, so the aliases **had to stay**. Both were right when written and **the condition has since been met**: the portal rebuilt on 2026-08-19 and its own changelog records that it *"dropped the deprecated `.rux-app-shell` aliases"*, `guide_runner` is archived, and the 2026-08-20 `v0.1.4` vendor sync carried the successors. **Grep protocol, before:** 4 in `rux-ui/css`, 3 in `tests`, 24 in `docs`, 1 in `.claude`, **0 in any page, `js/`, `rux-ui/js/`, `scheduler/` or `examples/`** — the only non-CSS occurrences were prose and the three test assertions, all listed before editing. **After:** 0 in CSS, 0 in tests as a *rule* (one `assert.doesNotMatch` guard remains by design), and the remaining `docs` hits are this ledger, the naming audit that found it, and the historical entries above. **The three test assertions were deleted, not repointed** — which corrects what naming.md step 9 predicted. `layout-contract.test.mjs` already asserted the successors (`.rux-app__body`, `.rux-app-view`) four lines earlier, so repointing would have duplicated live coverage to keep a dead name company; a `doesNotMatch` guard replaced them so the name cannot return by accident. **Three documents asserted the opposite and were corrected in the same change**, which is the reason the grep protocol reaches `docs/` and not just code: `layout-composition.md` published the MUST-NOT-USE rule, `audit/design-system-audit.md` H2 listed the block as a deliberate keep *because* consumers used it, and `.claude/skills/ponytail-review/SKILL.md` used it as its worked **classC** example under a promise that *"every example below is a real finding in this repository, checked at the line cited."* That example moved to `.rux-color-picker` (`rux-ui/css/base/form.css:642`, verified at the line), which is now the better illustration anyway — it is the candidate the owner **held**. **Cost:** 4 CSS rules, no tokens. `.rux-app` and `.rux-app-view` are untouched. **Deliberately not bundled:** `.rux-color-picker` (7 elements, 43 rules, 20 public tokens) is **held** pending the owner's confirmation that it was superseded rather than unfinished, and `.rux-tag`'s merge into `.rux-badge` changes call sites rather than deleting a name — both stay open in naming.md §8. |
| 24 | ~~Retire `.rux-color-picker`~~ **done (2026-08-22)** | Class C, executed on the owner's decision; `docs/foundations/naming.md` step 10. **The block was published, complete, and used by nothing** — a container plus seven elements, **15 rules** and **20 public `--rux-color-picker-*` tokens**, with zero references in any markup or script in this repository or in the consuming portal. §8.2 recorded it as *proposed rather than recommended* precisely because unused and obsolete are different claims, and it stayed held until the evidence separated them. **What settled it was mapping the three live colour sites**, not counting references again. `.rux-color-picker`'s anatomy — `__trigger` / `__preview` / `__popover` / `__option` — is a preset palette behind a popover, and both halves of that job now ship elsewhere: `.rux-color-swatches` does the palette **inline** (avatar colour, trip-bar colour) and `.rux-color-input` does **freeform entry** (vehicle colour, with a hex field). Nothing was left for the popover version to serve. **Grep protocol, before:** 66 in `rux-ui/css`, 1 in `scheduler` (a comment), 14 in `docs`, 2 in `.claude`, and **0 in any page, `js/`, `rux-ui/js/`, `scheduler/` markup or the consumer's own files**. **After:** 1 in `rux-ui/css` and 1 in `scheduler`, both explanatory comments naming the new home; **zero CSS rules and zero token declarations anywhere**. **Verified in the browser, because the removed block sat between two live ones in `form.css`:** `.rux-color-swatch` still renders 12 dots at 32×32 and 6px radius, `.rux-color-input` still flexes at an 8px gap with its hex field, and `--rux-color-picker-bg` now falls through to a probe's fallback, which is the token layer confirming the removal rather than the stylesheet being asked politely. **A count in §8.2 is corrected by this step**: it said 43 rules, which was `grep -c` counting *occurrences*. The block was **15 rules**. **`.claude/skills/ponytail-review/SKILL.md` needed its worked `classC` example replaced for the second time** — `.rux-app-shell` went in entry 23 and `.rux-color-picker` goes here. That is structural, not bad luck: the clearest illustration of *propose, never cut* is the best retirement candidate in the repository, which is the thing most likely to be cut next. The example is now `--rux-shadow-pressed`, chosen to be stable — dead, published, and on nobody's plan. |
| 25 | ~~Retire `--rux-duration-productive`~~ **done (2026-08-22)** | Class C; `docs/foundations/motion.md` step 4 proposed it. **One token, zero readers, and a positive reason to go beyond being unused.** It resolved `--rux-duration-*` into the Carbon `--rux-motion-*` scale, and `motion.md` Q1 established those two scales are **not** interchangeable — they time different things and carry different values. A dead alias between them is not neutral; it asserts a relationship the document had just disproved. **`--rux-motion-duration-moderate-02` was proposed alongside it and is deliberately kept.** That one is the fourth rung of a complete four-rung ladder, and **entry 16 of this ledger already decided this case**: *the 12 unused rungs of a complete scale or palette were kept — a design system ships whole ladders*. Retiring it would contradict a recorded decision and the same reasoning that kept the `full` vendor profile and the gallery-only components this month. Step 4 bundled the two because both measured as unread; unread is not the same claim as unwanted, and only one of them had a second reason. **Grep protocol, before:** 1 in `rux-ui/css` (the declaration), 1 in `tests`, 10 in `docs`, **0** `var()` readers anywhere and **0** consumer-owned references. **After:** 0 in CSS, 0 in tests, 0 in `scheduler`; the `docs` hits are this ledger and `motion.md`'s own record. **One test assertion went with it** — `layout-contract` pinned the alias's definition, which is a test defending a token that should not exist rather than a contract worth keeping. `motion-contract`'s two assertions on `moderate-02` stay, because that rung stays. |
| 26 | ~~Move the calendar tokens out of the portable layer~~ **done (2026-08-22)** | Class C, and it closes step 5's §4.7 follow-up. **This was a move before it was a rename, and the move is the defect.** All **22** declarations lived in `rux-ui/css/tokens.css` — the *portable* layer — so every consumer received a calendar's chrome tokens, and the portal's vendored copy carries **30** occurrences of names for a component it does not have. §4.7 is unambiguous that a calendar is a domain noun. They are now `--sched-calendar-*` in `scheduler/css/tokens.css`, both the `:root` block and its light-theme overrides. **Nothing portable read them**: the only four `var()` references inside `rux-ui/css` were calendar tokens referencing each other, all within `tokens.css`, so the move cut no dependency. 26 consumers updated, all in `scheduler/css` — `scheduler.css` 22 and `scheduler-app.css` 4. **Grep protocol, before:** 28 in `rux-ui/css`, 26 in `scheduler/css`, 2 in `docs`, **0 consumer-owned**. **After:** **zero** `--rux-calendar-*` anywhere in CSS. **Proved invisible rather than argued invisible**, which mattered because moving declarations between files moves them in the cascade: all **17 tokens × 2 themes = 34 resolved values are identical to the string**, captured from the running application before and after, and the old names resolve to nothing. The calendar renders — scheduler present, five trip bars, no unresolved custom properties. **Deliberately did not touch the `scope` marker**, step 5's other deferral, which is its own decision. |
