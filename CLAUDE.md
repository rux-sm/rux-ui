...
- A customer's itinerary document — PDF, email, photo, pasted notes — is entered
  through the scheduler's Claude connector, in the `rux-sm.github.io` repository:
  it reads the document and returns a link that opens a trip editor already
  filled, and only a person's Save writes the trip. The `process-itinerary` skill
  that did this through the Grid tab retired with that tab.
  Trip Draft v3 lives on in `worker/index.js`'s extract route, which `intake.html`
  uses; `docs/itinerary-prompt.md` and `docs/trip-import-schema-v3.json` are its
  prompt and schema.
...
- The staff pages (`index.html`, `intake.html`) sign in with a staff account (`js/components/staff-sign-in.js`), and `js/data/supabase.js` holds every data request on those pages until that sign-in succeeds; the public link pages (driver, maintenance, document) use the key in page source with no sign-in, and the database lets that key call only the link pages' token-checked functions. Do not add Social Security numbers or comparable identity secrets to tables this client reads — they stay in the payroll/HR system that already holds them (decided 2026-08-21; destinations and phone numbers are fine, an SSN turns a leak into identity theft and a reportable breach). If this is ever revisited, such a field is read only through a `SECURITY DEFINER` function that checks for staff, revoked from `anon`.
