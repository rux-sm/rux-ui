...
/* The same two blocks, comments intact. The provenance test reads the ramp
   notes themselves, so it cannot use the stripped blocks above. */
const THEMES = [
	["dark", SRGB.slice(0, LIGHT_AT)],
	["light", SRGB.slice(LIGHT_AT)],
];
...
test("every step records the measurement it came from", () => {
	/* color.md rule 2.10. A step is either MEASURED — converted from a source
	   colour, which it cites beside it so the conversion can be rechecked — or
	   COMPUTED, generated by a ramp, which it states once at the head of the
	   scale. Without one of the two the provenance is gone and the next reader
	   cannot check the value, which is how a measured system decays into a
	   chosen one that has forgotten it chose.

	   The computed branch arrived with step 34's even ramp: the seven hue
	   scales stopped being conversions of the catalog's HSL, so requiring an
	   `hsl()` citation there was demanding provenance that no longer exists —
	   and writing one anyway would have been fabricating it. `gray` is still
	   the catalog's and still cites, which is why both branches are live.

	   THE RAMP NOTE IS VERIFIED, NOT TRUSTED. A comment nothing checks is
	   decoration, and this rule exists precisely because unchecked provenance
	   rots. Every step a scale declares is recomputed from its own note and
	   must match, so editing a value without editing the note fails here. */
	const undocumented = [];
	const contradicted = [];

	for (const [label, block] of THEMES) {
		for (const scale of [...HUES, "gray"]) {
			const at = block.indexOf(`--rux-${scale}-100:`);
			if (at === -1) continue;
			const note = /COMPUTED, not measured[\s\S]{0,400}?\*\//.exec(
				block.slice(Math.max(0, at - 500), at),
			);

			if (!note) {
				/* Measured branch: every step cites its source inline. */
				for (const step of STEPS) {
					for (const l of block.match(
						new RegExp(`--rux-${scale}-${step}:[^\\n]*`, "g"),
					) ?? []) {
						if (!/\/\* hsl\(/.test(l))
							undocumented.push(`${label} ${l.trim().slice(0, 60)}`);
					}
				}
				continue;
			}

			/* Computed branch: recompute all ten steps from the note. */
			const text = note[0];
			const ramp = /L ([\d,]+) at C ([\d.]+), H (\d+)/.exec(text);
			if (!ramp) {
				undocumented.push(`${label} --rux-${scale}: ramp note is unreadable`);
				continue;
			}
			const Ls = ramp[1].split(",");
			const baseC = ramp[2];
			const H = ramp[3];
			const off = new Map();
			/* Chroma always carries a decimal, so anchoring on one keeps the
				   sentence's trailing period out of the captured value. */
			for (const [, step, c] of text.matchAll(/(\d+) C (\d*\.\d+)/g))
				off.set(step, c);

			if (Ls.length !== STEPS.length) {
				undocumented.push(
					`${label} --rux-${scale}: note lists ${Ls.length} steps, scale has ${STEPS.length}`,
				);
				continue;
			}
			STEPS.forEach((step, n) => {
				const declared = new RegExp(
					`--rux-${scale}-${step}:\\s*(oklch\\([^)]*\\))`,
				).exec(block);
				if (!declared) return;
				const want = `oklch(${Ls[n]}% ${off.get(String(step)) ?? baseC} ${H})`;
				if (declared[1] !== want)
					contradicted.push(
						`${label} --rux-${scale}-${step}: note says ${want}, file says ${declared[1]}`,
					);
			});
		}
	}
